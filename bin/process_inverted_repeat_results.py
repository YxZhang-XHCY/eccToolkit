...
def process_blast_results(csv_input_file, csv_output_file):
    ...
    df_filtered['s_real_start'] = pd.to_numeric(df_filtered['subject_id'].str.extract(r'-(\d+)')[0], errors='coerce') - df_filtered['s_end'] + 1
    df_filtered['s_real_end'] = pd.to_numeric(df_filtered['subject_id'].str.extract(r'-(\d+)')[0], errors='coerce') - df_filtered['s_start'] + 1
