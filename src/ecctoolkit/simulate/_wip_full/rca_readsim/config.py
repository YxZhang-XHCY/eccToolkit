...
class RCAKineticsParams:
    ...
    reaction_time_min: float = 90.0  # 1.5h
...
class OutputScaleParams:
    ...
    仅支持覆盖度模式：指定目标覆盖倍数，自动换算为 reads 数量。
    实际 reads 由外部工具 (ART/PBSIM2) 根据覆盖度生成。

    两阶段自适应采样：
    1. 初始采样 initial_ratio 比例的 reads
    2. 分析覆盖度，补充到 min_coverage
    3. 计算差值，继续采样或下采样到目标
    """
    Cov_NGS: Union[int, float] = 30.0       # NGS 目标覆盖度
    Cov_HiFi: Union[int, float] = 30.0      # HiFi 目标覆盖度
    Cov_ONT: Union[int, float] = 30.0       # ONT 目标覆盖度

    # 自适应采样参数
    adaptive_sampling: bool = True          # 是否启用自适应采样（确保每个eccDNA有覆盖）
    min_coverage: float = 3.0               # 每个 eccDNA 的最小覆盖度
...
class FixedConstants:
    ...
    # HiFi分支处理参数
    hifi_branched_drop_prob: float = 0.0       # 含分支分子被丢弃的概率
    hifi_branched_shorten_prob: float = 0.0    # 含分支分子产生更短片段的概率
    hifi_branched_target_mean_scale: float = 0.7  # 缩短时的目标长度比例
