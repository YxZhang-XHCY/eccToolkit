...
def validate_non_overlapping_junctions(junctions: List[ChimeraJunction]) -> List[ChimeraJunction]:
    ...
    for i in range(1, len(junctions)):
        ...
        if prev_end > curr_start:
            # 发现重叠，记录警告并移除
            logger.warning(
                "Overlapping chimera junctions detected: "
                "junction at %d (end=%d) overlaps with junction at %d. "
                "Removing the overlapping junction.",
                prev.position, prev_end, curr_start
            )
            # 跳过当前junction（保留前一个）
            continue
...
class LinearMoleculePool:
    ...
    def get_subsequence(self, mol: LinearMolecule, start: int, length: int) -> str:
        """获取分子指定区间序列（避免重建整条分子序列）"""
        return mol.get_subsequence(self.ecc_db, start, length)
    ...
    def to_dataframe(
        ...
        read_length: int = 150,
        original_ecc_total_length: int = None
    ) -> 'pd.DataFrame':
        """
        将分子池转换为 DataFrame 格式，供 fqsim 使用

        Args:
            include_coverage: 是否包含覆盖度列（tempcov）
            include_truth: 是否包含 eccDNA 来源追踪信息
            target_coverage: 目标覆盖度（如果指定，将基于此计算 tempcov）
            read_length: read 长度（用于计算 tempcov）
            original_ecc_total_length: 原始 eccDNA 的总长度（用于正确计算 tempcov）

        Returns:
            DataFrame with columns: id, seq, length, tempcov, ecc_ids, ...
        """
        import pandas as pd

        # 计算抽样概率：当 target_coverage 和 original_ecc_total_length 都有效时
        # 使用抽样策略，避免每个片段的覆盖度太低
        use_sampling = False
        ref_length = original_ecc_total_length if original_ecc_total_length else self.total_length
        if include_coverage and target_coverage is not None and target_coverage > 0:
            pool_length = self.total_length
            # 计算目标总 bases 和 pool 总长度的比值
            target_bases = target_coverage * ref_length
            if pool_length > 0 and target_bases < pool_length:
                # 需要抽样：先收集所有片段，然后随机抽样
                use_sampling = True
                sampling_ratio = target_bases / pool_length
        ...
        if use_sampling:
            ...
            # 按权重随机抽样，确保总长度接近目标
            # 使用概率抽样：每个片段的抽样概率与其权重成正比
            sampled_indices = []
            sampled_total_length = 0
            for frag in all_fragments:
                # 抽样概率 = 权重 * 抽样比例 * 分子数量
                prob = min(1.0, frag['weight'] * sampling_ratio * len(self.molecules))
                if rng.random() < prob:
                    sampled_indices.append(frag['idx'])
                    sampled_total_length += frag['length']

            # 计算被抽样片段应该使用的 tempcov
            # tempcov = target_coverage * original_ecc_length / sampled_total_length
            if sampled_total_length > 0:
                adjusted_tempcov = target_coverage * ref_length / sampled_total_length
            else:
                adjusted_tempcov = target_coverage

            # 生成被抽样片段的记录
            for frag in all_fragments:
                if frag['idx'] not in sampled_indices:
                    continue
                mol = frag['mol']
                seq = frag['seq']
                record = {
                ...
                if include_coverage:
                    record['tempcov'] = max(0.1, float(adjusted_tempcov))
                if include_truth:
                    ecc_ids = set()
                    for seg in mol.segments:
                        if seg.segment_type != SegmentType.BACKGROUND:
                            ecc_ids.add(seg.ecc_id)
                    record['ecc_ids'] = '|'.join(sorted(ecc_ids)) if ecc_ids else ''
                    record['repeat_count'] = mol.repeat_count
                    record['source_ecc_length'] = mol.source_ecc_length
                    record['has_chimera'] = mol.has_chimera
                    record['is_background'] = mol.is_background
                    record['source_graph_id'] = mol.source_graph_id
                    if mol.is_background:
                        record['background_region'] = (
                            f"{mol.background_chrom}:{mol.background_start}-{mol.background_end}"
                            if mol.background_chrom else ''
                        )
                    else:
                        record['background_region'] = ''
            ...
            for idx, mol in enumerate(self.molecules):
                ...
                if include_coverage:
                    if target_coverage is not None and target_coverage > 0:
                        # 直接使用目标覆盖度（pool 长度 <= 目标 bases 时）
                        record['tempcov'] = float(target_coverage)
                    else:
                        # 后向兼容：基于分子权重计算相对覆盖度
                        record['tempcov'] = float(self.weights[idx] * len(self.molecules))
    ...
    def to_csv(
        ...
        """
        将分子池导出为 CSV 文件，供 fqsim 使用

        Args:
            output_path: 输出 CSV 文件路径
            include_coverage: 是否包含覆盖度列
            include_truth: 是否包含 eccDNA 来源追踪信息
            target_coverage: 目标覆盖度（如果指定，将基于此计算 tempcov）
            read_length: read 长度（用于计算 tempcov）
            original_ecc_total_length: 原始 eccDNA 的总长度（用于正确计算 tempcov）
        ...
        df = self.to_dataframe(
            include_coverage, include_truth, target_coverage, read_length,
            original_ecc_total_length
        )
