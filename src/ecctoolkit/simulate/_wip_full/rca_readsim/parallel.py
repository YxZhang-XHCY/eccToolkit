...
def _extract_ecc_ids_from_compartments(compartments: List[Any]) -> set:
    """从 compartments 中提取所有涉及的 ecc_id"""
    ecc_ids = set()
    for comp in compartments:
        # Compartment.ecc_instances 是 List[Tuple[str, int]]，即 (ecc_id, instance_index)
        ecc_instances = getattr(comp, 'ecc_instances', [])
        for item in ecc_instances:
            if isinstance(item, tuple) and len(item) >= 1:
                ecc_id = item[0]
                if ecc_id:
                    ecc_ids.add(ecc_id)
            elif hasattr(item, 'ecc_id'):
                # 兼容可能的对象形式
                ecc_id = getattr(item, 'ecc_id', None)
                if ecc_id:
                    ecc_ids.add(ecc_id)
    return ecc_ids


def _build_subset_ecc_db(ecc_db: Dict[str, Any], ecc_ids: set) -> Dict[str, Any]:
    """构建只包含指定 ecc_ids 的子集 ecc_db"""
    return {k: v for k, v in ecc_db.items() if k in ecc_ids}
...
def parallel_generate_rca(
    ...
    优化：每个 worker 只接收它需要的 ecc_db 子集，减少数据传输开销
    """
    ...
    tasks = []
    total_ecc_count = len(ecc_db)
    subset_sizes = []

    for idx, chunk in enumerate(chunks):
        if chunk:
            # 为每个 worker 构建只包含它需要的 eccDNAs 的子集
            ecc_ids = _extract_ecc_ids_from_compartments(chunk)
            subset_db = _build_subset_ecc_db(ecc_db, ecc_ids)
            subset_sizes.append(len(subset_db))
            tasks.append((chunk, subset_db, config, seed, idx))
    ...
    logger.info(
        f"  Distributing {len(compartments)} compartments to {worker_count} workers "
        f"(avg {avg_subset_size:.0f}/{total_ecc_count} eccDNAs per worker)"
    )

    with mp.Pool(worker_count) as pool:
        # 使用 apply_async 以便能够追踪进度
        async_results = [pool.apply_async(_worker_generate_rca, task) for task in tasks]
        ...
        for async_result in async_results:
            result = async_result.get()  # 等待单个任务完成
            results.append(result)
            completed += 1
            # 每完成 25% 时报告进度
            progress_pct = int(100 * completed / total)
            if progress_pct >= last_reported + 25 or completed == total:
                logger.info(f"  RCA progress: {completed}/{total} workers done ({progress_pct}%)")
...
def parallel_inject_chimera(
    ...
    worker_count = min(num_workers, len(tasks))
    logger.info(
        f"  Distributing {len(molecules)} molecules ({len(groups)} compartments) to {worker_count} workers"
    )

    with mp.Pool(worker_count) as pool:
        async_results = [pool.apply_async(_worker_inject_chimera, task) for task in tasks]
        ...
        for async_result in async_results:
            ...
            progress_pct = int(100 * completed / total)
            if progress_pct >= last_reported + 25 or completed == total:
                logger.info(f"  Chimera progress: {completed}/{total} workers done ({progress_pct}%)")
                last_reported = progress_pct
...
def _extract_ecc_ids_from_molecules(molecules: List[Any]) -> set:
    """从 molecules 中提取所有涉及的 ecc_id"""
    ecc_ids = set()
    for mol in molecules:
        # 从 source_ecc 获取
        source_ecc = getattr(mol, 'source_ecc', None)
        if source_ecc:
            ecc_id = getattr(source_ecc, 'id', None)
            if ecc_id:
                ecc_ids.add(ecc_id)
        # 从 branches 获取
        for branch in getattr(mol, 'branches', []) or []:
            seg = getattr(branch, 'segment', None)
            if seg:
                ecc_id = getattr(seg, 'ecc_id', None)
                if ecc_id:
                    ecc_ids.add(ecc_id)
    return ecc_ids
...
def parallel_debranch(
    ...
    for idx, chunk in enumerate(chunks):
        if chunk:
            # 为每个 worker 构建只包含它需要的 eccDNAs 的子集
            ecc_ids = _extract_ecc_ids_from_molecules(chunk)
            subset_db = _build_subset_ecc_db(ecc_db, ecc_ids)
            subset_sizes.append(len(subset_db))
            tasks.append((chunk, subset_db, config, seed, idx))
    ...
    avg_subset_size = sum(subset_sizes) / len(subset_sizes) if subset_sizes else 0
    logger.info(
        f"  Distributing {len(molecules)} molecules to {worker_count} workers "
        f"(avg {avg_subset_size:.0f}/{total_ecc_count} eccDNAs per worker)"
    )

    with mp.Pool(worker_count) as pool:
        async_results = [pool.apply_async(_worker_debranch, task) for task in tasks]
        ...
        for async_result in async_results:
            ...
            progress_pct = int(100 * completed / total)
            if progress_pct >= last_reported + 25 or completed == total:
                logger.info(f"  Debranch progress: {completed}/{total} workers done ({progress_pct}%)")
                last_reported = progress_pct
...
def _rebuild_pool(pool_data: Dict[str, Any]):
    ...
    for mol_data in pool_data['molecules']:
        ...
        active_branches = []
        for branch_data in mol_data.get('active_branches', []) or []:
            seg_data = branch_data.get('branch_segment') or {}
            seg = Segment(
                ecc_id=seg_data['ecc_id'],
                ecc_offset=seg_data['ecc_offset'],
                length=seg_data['length'],
                strand=Strand(seg_data['strand']),
                segment_type=SegmentType(seg_data['segment_type']),
                parent_offset=seg_data.get('parent_offset'),
            )
            active_branches.append(
                ActiveBranchInfo(
                    anchor_pos=branch_data['anchor_pos'],
                    branch_segment=seg,
    ...
    # 创建分子池
    weight_by_length = bool(pool_data.get('weight_by_length', True))
    return LinearMoleculePool(molecules, ecc_db, weight_by_length=weight_by_length)
...
def serialize_pool(pool) -> Dict[str, Any]:
    ...
    for mol in pool.molecules:
        ...
        active_branches_data = []
        for branch_info in getattr(mol, 'active_branches', []) or []:
            seg = branch_info.branch_segment
            active_branches_data.append({
                'anchor_pos': branch_info.anchor_pos,
                ...
                'branch_segment': {
                    ...
                    'strand': seg.strand.value,
                    'segment_type': seg.segment_type.value,
                    'parent_offset': seg.parent_offset,
                }
            })
        ...
        mol_data = {
            ...
            'active_branches': active_branches_data,
            'junction_covered_possible': mol.junction_covered_possible
        }
        molecules_data.append(mol_data)

    return {
        'weight_by_length': bool(getattr(pool, 'weight_by_length', True)),
        'ecc_db': ecc_db_data,
