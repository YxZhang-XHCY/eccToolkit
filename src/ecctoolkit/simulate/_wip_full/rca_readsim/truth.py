...
class TruthWriter:
    ...
    def _read_to_record(self, read: SequencedRead) -> dict:
        ...
        if self.format == "jsonl":
            # JSONL: 直接存储为列表/字典，不序列化为字符串
            record["repeat_count_by_source"] = repeat_by_source
            record["segments"] = segments_data
        else:
            # TSV: 序列化为 JSON 字符串
...
class SegmentTsvWriter:
    ...
    def _get_writer(self, platform: str) -> TextIO:
        ...
        if key not in self._writers:
            ...
            self._writers[key] = open(tsv_path, self.mode, encoding="utf-8")
        ...
        with open(self.input_path, 'r', encoding='utf-8') as f:
            if self.format == "jsonl":
                for line in f:
                    ...
                    if line:
                        row = json.loads(line)
                        # 兼容旧格式：如果存在 segments_json 但没有 segments，解析它
                        if 'segments' not in row and 'segments_json' in row and row['segments_json']:
                ...
                for row in reader:
                    ...
                    for key in ['has_inter_chimera',
                        ...
                        if key in row and row[key]:
                            row[key] = int(row[key])
                    # repeat_count_truth 可能是浮点数
                    if 'repeat_count_truth' in row and row['repeat_count_truth']:
                        row['repeat_count_truth'] = float(row['repeat_count_truth'])
                    records.append(row)
