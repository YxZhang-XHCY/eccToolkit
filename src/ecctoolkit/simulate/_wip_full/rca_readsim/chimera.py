...
class ChimeraInjector:
    ...
    def _find_non_overlapping_breakpoint(
        self,
        ...
        """
        找到一个不与已有 chimera junctions 重叠的断点位置

        Args:
            recipient: 受体分子图
            chim_len: 要插入的 chimera 长度
            max_attempts: 最大尝试次数

        Returns:
            非重叠的断点位置，如果找不到则返回 None
        """
        T_rec = recipient.trunk_length
        if T_rec <= 0:
            return None

        existing_junctions = getattr(recipient, 'chimera_junctions', []) or []
        if not existing_junctions:
            # 没有已有的 junctions，直接采样
            return self._sample_breakpoint(T_rec)

        # 构建已占用区间列表 [(start, end), ...]
        occupied = []
        for junction in existing_junctions:
            start = junction.position
            end = start + junction.donor_segment.length
            occupied.append((start, end))
        occupied.sort()

        for _ in range(max_attempts):
            breakpoint = self._sample_breakpoint(T_rec)
            new_end = breakpoint + chim_len

            # 检查是否与任何已有区间重叠
            overlaps = False
            for (occ_start, occ_end) in occupied:
                # 检查 [breakpoint, new_end) 与 [occ_start, occ_end) 是否重叠
                if not (new_end <= occ_start or breakpoint >= occ_end):
                    overlaps = True
                    break
    ...
    def inject_chimera(
        ...
        # 计算chimera长度（需要先知道长度才能检查重叠）
        chim_len = self.compute_chimera_length(T_don)
        if chim_len <= 0:
            return False

        # 在受体主干上找一个不重叠的断点
        breakpoint = self._find_non_overlapping_breakpoint(recipient, chim_len)
