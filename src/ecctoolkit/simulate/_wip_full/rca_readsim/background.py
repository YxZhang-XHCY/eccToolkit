...
class BackgroundDNAGenerator:
    ...
    def _open_fasta(self):
        ...
        try:
            self._fasta = ps.Fastafile(fasta_path)
            return self._fasta
        except Exception:
            # Try to build FASTA index if missing (writable references only).
            try:
                ps.faidx(fasta_path)
                self._fasta = ps.Fastafile(fasta_path)
                return self._fasta
                ...
                logger.warning(
                    f"Failed to open FASTA via pysam (falling back to in-memory load): {e}"
                )
                self._fasta = None
                return None
    ...
    def _load_reference(self):
        ...
        if fasta is not None:
            self.chromosomes = [
                ChromosomeInfo(name=name, length=int(length))
            ...
            self.total_genome_length = cumsum
            logger.info(
                f"Loaded {len(self.chromosomes)} chromosomes via FASTA index, "
                f"total length: {self.total_genome_length:,} bp"
            )
            return

        # Fallback: load whole reference into memory (no FASTA index available).
        current_name = None
        current_seq = []
                ...
                if line.startswith('>'):
                    if current_name is not None and current_seq:
                        ...
                        self.chromosomes.append(
                            ChromosomeInfo(name=current_name, length=len(seq), sequence=seq)
                        )
        ...
        logger.info(
            f"Loaded {len(self.chromosomes)} chromosomes (in-memory), "
            f"total length: {self.total_genome_length:,} bp"
        )
    ...
    def _sample_region(self, length: int) -> Tuple[ChromosomeInfo, int, int]:
        ...
        chrom_idx = bisect_right(self.chrom_cumsum, rand_pos)
        chrom_idx = max(0, min(chrom_idx, len(self.chromosomes) - 1))
    ...
    def generate_fragment(self, fragment_id: int) -> LinearMolecule:
        ...
        # 提取序列
        if self._fasta is not None:
            sequence = self._fasta.fetch(chrom.name, start, end).upper()
        else:
            if chrom.sequence is None:
                raise RuntimeError(
                    "BackgroundDNAGenerator has no FASTA handle and no in-memory reference sequence"
                )
...
def calculate_background_count(
    ...
    NOTE: 此函数现在仅用于预生成 chimera 背景供体池。
    实际的背景 reads 数量由 reads.py 中的独立采样逻辑控制，
    以确保最终输出中背景 reads 占比精确等于 background_ratio。
