...
class RCAEngine:
    ...
    def generate_molecule(
        ...
        if num_branches == 0:
            ...
            # 批量采样所有锚点位置
            anchor_positions = self.rng.integers(0, T, size=num_branches)
            ...
            ln_r = self.rng.normal(mu_br, sigma_br, size=num_branches)
            r = np.exp(ln_r)
            ...
            branch_lengths = np.clip((r * base_len).astype(int), 1, base_len)

            # 批量采样方向翻转
            p_flip = self.config.constants.p_flip
            if p_flip > 0:
                flip_mask = self.rng.random(size=num_branches) < p_flip
            else:
                flip_mask = np.zeros(num_branches, dtype=bool)

            # 创建分支对象
            branches = []
            for b_idx in range(num_branches):
                anchor_pos = int(anchor_positions[b_idx])
                branch_len = int(branch_lengths[b_idx])
                branch_offset = anchor_pos % L
                strand = Strand.REVERSE if flip_mask[b_idx] else Strand.FORWARD
