...
def create_visualizations(filtered_df, results):
    ...
    # 4. 序列长度分布
    axes[1, 1].scatter(filtered_df['seq_length'], filtered_df['total_anno_percent'], alpha=0.6)
    axes[1, 1].set_xlabel('Sequence Length (bp)')
    axes[1, 1].set_ylabel('Total anno_Percent')
    axes[1, 1].set_title('Sequence Length vs Total anno_Percent')
...
def main():
    ...
    try:
        ...
        print("\n分析完成! 请查看生成的文件:")
        print("- high_coverage_eccdna.csv: 筛选后的eccDNA数据")
