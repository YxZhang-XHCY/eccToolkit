...
def create_visualizations(df_motif, df_anno):
    ...
    fig.suptitle('Distribution of Single/Compound Transposon Percentage', fontsize=16, fontweight='bold')
    ...
    axes[0,0].axvline(df_motif['motif_percent'].mean(), color='red', linestyle='--', label=f'Mean: {df_motif["motif_percent"].mean():.1f}%')
    axes[0,0].legend()
    ...
    axes[1,0].axvline(df_anno['anno_Percent'].mean(), color='red', linestyle='--', label=f'Mean: {df_anno["anno_Percent"].mean():.1f}%')
    axes[1,0].legend()
