...
def load_window_candidates(window_files, threshold_factor=2.0):
    ...
    for scale, file in window_files.items():
        df = pd.read_csv(file)
        sample_cols = [c for c in df.columns if c not in ['Chrom','WindowStart','WindowEnd']]
    ...
    for chrom, group in all_cand.groupby('Chrom'):
        intervals = group[['WindowStart','WindowEnd']].sort_values('WindowStart').values
        s, e = None, None
...
def get_highres_signal(chrom, start, end, dfs, bin_size=1000):
    ...
    for df in dfs:
        df1 = df[(df['eChr']==chrom)&(df['eEnd']>=search_start)&(df['eStart']<=search_end)]
...
def permutation_test(chrom, s, e, dfs, chrom_length, n_perm=1000):
    ...
    置换热点区间并统计信号，返回p值
    """
    obs = np.mean([len(df[(df['eChr']==chrom)&(df['eEnd']>=s)&(df['eStart']<=e)]) for df in dfs])
    region_len = e-s
    null = []
    for _ in range(n_perm):
        ...
        ps = shift
        pe = shift+region_len
        perm_val = np.mean([len(df[(df['eChr']==chrom)&(df['eEnd']>=ps)&(df['eStart']<=pe)]) for df in dfs])
...
def main():
    ...
    for idx, row in tqdm(candidates.iterrows(), total=len(candidates)):
        ...
        regions = find_hotspot_regions(signal_track, bins, start, end, args.min_hotspot_size)
        for s, e in regions:
            mean_signal, enrichment, consistency, min_sig, max_sig = compute_hotspot_stats(chrom, s, e, dfs)
            pval = None
            if args.permutation_test:
                pval = permutation_test(chrom, s, e, dfs, chrom_length, args.n_permutations)
            results.append({
                'chrom': chrom, 'start': int(s), 'end': int(e), 'length': int(e-s),
                'mean_signal': mean_signal, 'enrichment': enrichment,
                'consistency': consistency, 'min_signal': min_sig, 'max_signal': max_sig,
                'category': classify_hotspot(consistency, enrichment),
                'boundary_pvalue': pval
            })
    # 输出结果
    dfres = pd.DataFrame(results)
    if not os.path.exists(os.path.dirname(args.output_prefix)) and os.path.dirname(args.output_prefix):
    ...
    with open(f'{args.output_prefix}_hotspots.bed','w') as f:
        for _, row in dfres.iterrows():
            ...
            f.write(f'{row["chrom"]}\t{int(row["start"])}\t{int(row["end"])}\thotspot_{row["category"]}\t{score}\t.\n')
    print('分析完成。结果见：')
