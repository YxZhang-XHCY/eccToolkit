...
def main(input_file, output_file):
    ...
    df[['chr', 'seq_start', 'seq_end', 'seq_length']] = df['seqname'].apply(parse_seqname)
    df[['motif', 'motif_start', 'motif_end']] = df['attribute'].apply(parse_attribute)

    # 计算 motif_length
    df['motif_length'] = df.apply(
        lambda row: int(row['motif_end']) - int(row['motif_start']) + 1 if pd.notnull(row['motif_start']) and pd.notnull(row['motif_end']) else None, axis=1
    )

    # motif_length / seq_length * 100, 保留两位小数
    df['motif_percent'] = df.apply(
        lambda row: round(row['motif_length'] / row['seq_length'] * 100, 2)
        if pd.notnull(row['motif_length']) and pd.notnull(row['seq_length']) and row['seq_length'] != 0 else None,
        axis=1
    )

    # 所有整数列处理为 int（忽略 None），如果不为 None
    for col in ['seq_start', 'seq_end', 'seq_length', 'motif_start', 'motif_end', 'motif_length']:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input', '-i', type=str, required=True, help='输入文件名（csv）')
    parser.add_argument('--output', '-o', type=str, required=True, help='输出文件名（csv）')
