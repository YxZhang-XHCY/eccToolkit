#!/usr/bin/env python3
"""
eccDNA TAD Enrichment Analysis Script v1.0
Specialized version for TAD boundary-eccDNA enrichment analysis with customizable output prefix
"""
...
class TADEccDNAAnalysis:
    def __init__(self, args):
        ...
        self.buffer_sizes = args.buffer_sizes
    ...
    def load_tad_boundaries(self):
        ...
        try:
            tad_df = pd.read_csv(self.tad_file, sep='\t', header=None,
        ...
        # Filter chromosomes
        tad_df = tad_df[~tad_df['chr'].str.contains('chrM|_')]
        
        logger.info(f"Total TAD boundaries: {len(tad_df)}")
        logger.info(f"Strong boundaries: {len(tad_df[tad_df['boundary_type'] == 'Strong'])}")
        logger.info(f"Weak boundaries: {len(tad_df[tad_df['boundary_type'] == 'Weak'])}")
    ...
    def analyze_tad_enrichment(self, eccdna_file, tad_df, n_shuffles=1000):
        ...
        for boundary_type in boundary_types:
            for buffer_size in self.buffer_sizes:
                ...
                window_file = self.create_tad_windows(tad_df, buffer_size, boundary_type)
                observed = self.run_bedtools_intersect(eccdna_file, window_file)
                logger.info(f"  Observed count: {observed}")
                
                null_counts = self.parallel_shuffle_and_intersect(eccdna_file, window_file, 
        ...
        results_df.to_csv(self.results_dir / f'{self.prefix}_tad_enrichment_results.csv', index=False)
    ...
    def export_null_distributions(self, null_distributions):
        ...
        df.to_csv(self.plot_data_dir / f'{self.prefix}_tad_null_distributions.csv', index=False)
    ...
    def plot_tad_sensitivity(self, results_df):
        ...
        buffer_sizes_kb = [bs/1000 for bs in self.buffer_sizes]  # Convert to kb
        colors = {'Strong': 'red', 'Weak': 'blue', 'All': 'green'}
        
        # Export data for external plotting
        plot_data = results_df[['boundary_type', 'buffer_size', 'fold_enrichment', 
                               'null_mean', 'null_sd', 'p_value', 'q_value']].copy()
        plot_data['buffer_kb'] = plot_data['buffer_size'] / 1000
        plot_data.to_csv(self.plot_data_dir / f'{self.prefix}_tad_sensitivity_data.csv', index=False)
        ...
        for boundary_type in ['Strong', 'Weak', 'All']:
            ...
            plt.errorbar(buffer_sizes_kb, fold_enrichments,
                        yerr=[fold_enrichments - ci_lower, ci_upper - fold_enrichments],
                        marker='o', markersize=8, linewidth=2, capsize=5,
                        label=boundary_type, color=colors[boundary_type])
        ...
        plt.tight_layout()
        plt.savefig(self.figures_dir / f'{self.prefix}_tad_sensitivity_analysis.png', dpi=300)
        plt.close()
    ...
    def plot_summary_heatmap(self, tad_results):
        """Create a summary heatmap of TAD enrichment results"""
        plt.figure(figsize=(10, 8))
        ...
        # TAD results
        for _, row in tad_results.iterrows():
        ...
        heatmap_df = pd.DataFrame({
            'category': labels,
            'fold_enrichment': heatmap_data,
            'q_value': p_values
        })
        heatmap_df.to_csv(self.plot_data_dir / f'{self.prefix}_tad_enrichment_summary.csv', index=False)
        ...
        sns.heatmap(data_matrix, annot=annot_matrix, fmt='', cmap='RdYlBu_r',
                   yticklabels=labels, xticklabels=['Fold Enrichment'],
                   cbar_kws={'label': 'Fold Enrichment'})
        
        plt.title('TAD Boundary eccDNA Enrichment Summary\n(* q<0.05, ** q<0.01, *** q<0.001)')
        plt.tight_layout()
        plt.savefig(self.figures_dir / f'{self.prefix}_tad_enrichment_heatmap.png', dpi=300)
        plt.close()

    def generate_report(self, tad_results, total_eccdna):
        """Generate TAD-specific analysis report"""
        timestamp = pd.Timestamp.now().strftime('%Y-%m-%d %H:%M')
        
        report = f"""# eccDNA TAD Enrichment Analysis Report
...
- **TAD boundaries analyzed**: Strong, Weak, and All combined
- **Buffer sizes tested**: {', '.join([f'{bs/1000:.0f}kb' for bs in self.buffer_sizes])}

## Key Findings

### TAD Boundary Enrichment
"""
        sig_tad_count = sum(tad_results['q_value'] < 0.05)
        report += f"\n**Significant enrichments (q < 0.05)**: {sig_tad_count} out of {len(tad_results)}\n\n"
...
### Results
- `{self.prefix}_tad_enrichment_results.csv`: TAD boundary enrichment statistics
- `{self.prefix}_tad_analysis_report.md`: This report

### Figures
- `{self.prefix}_quality_control.png`: eccDNA distribution and size plots
- `{self.prefix}_tad_sensitivity_analysis.png`: TAD enrichment vs buffer size
- `{self.prefix}_tad_enrichment_heatmap.png`: Summary heatmap with significance
...
- `{self.prefix}_tad_sensitivity_data.csv`: TAD enrichment vs buffer size
- `{self.prefix}_tad_enrichment_summary.csv`: All enrichment values
- `{self.prefix}_tad_null_distributions.csv`: Null distribution data
...
### TAD Boundary Types:
- **Strong**: High-confidence TAD boundaries
- **Weak**: Lower-confidence TAD boundaries
- **All**: Combined strong and weak boundaries
...
---
*Report generated automatically by the eccDNA TAD Enrichment Analysis Pipeline*
"""
        
        with open(self.results_dir / f'{self.prefix}_tad_analysis_report.md', 'w') as f:
            f.write(report)
        
        logger.info(f"\nReport saved to: {self.results_dir / f'{self.prefix}_tad_analysis_report.md'}")

    def run_analysis(self):
        """Run TAD analysis pipeline"""
        print("="*60)
        print("eccDNA TAD Enrichment Analysis Pipeline v1.0")
        print("="*60)
        ...
        try:
            ...
            # Run TAD enrichment analysis
            tad_results = self.analyze_tad_enrichment(eccdna_file, tad_df, self.n_shuffles)
            
            # Create summary heatmap
            self.plot_summary_heatmap(tad_results)
            
            # Generate report
            total_eccdna = len(eccdna_df)
            self.generate_report(tad_results, total_eccdna)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="eccDNA TAD Enrichment Analysis Pipeline",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic TAD analysis with default buffer sizes
  %(prog)s -i sample1.csv sample2.csv -t TAD_boundaries.bed
  
  # Custom output prefix
  %(prog)s -i eccDNA.csv -t TAD.bed -p MCF7_analysis
  
  # Custom buffer sizes (in bp)
  %(prog)s -i eccDNA.csv -t TAD.bed -p experiment1 --buffer-sizes 10000 25000 50000 100000
  
  # Custom parameters
  %(prog)s -i eccDNA.csv -t TAD.bed -p sample1 -n 10000 --cores 32
  
  # Specify output directory
  %(prog)s -i eccDNA.csv -t TAD.bed -p sample1 -w ./results/
        """
    )
    ...
    parser.add_argument('-t', '--tad', required=True, 
    ...
    parser.add_argument('--buffer-sizes', nargs='+', type=int, 
                       default=[25000, 50000, 100000],
                       help='Buffer sizes around TAD boundaries in bp (default: 25000 50000 100000)')
...
def main():
    ...
    analyzer = TADEccDNAAnalysis(args)
