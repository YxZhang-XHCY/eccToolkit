...
def convert_csv_to_bed(csv_file, bed_file):
    ...
    try:
        ...
        if 'eChr' not in df.columns or 'eStart' not in df.columns or 'eEnd' not in df.columns:
            raise ValueError(f"CSV file must contain eChr, eStart, and eEnd columns. Found: {list(df.columns)}")
...
def run_bedtools_makewindows(fai_file, window_size, windows_file):
    ...
    command = [
        'bedtools', 'makewindows', '-g', fai_file, '-w', str(window_size)
    ]
    try:
...
def run_bedtools_intersect(windows_file, eccdna_bed_file, result_file):
    ...
    command = [
        'bedtools', 'intersect', '-a', windows_file, '-b', eccdna_bed_file, '-c'
    ]
    try:
...
def merge_binned_data(binned_data_list):
    ...
    for data in binned_data_list[1:]:
        merged_data = pd.merge(merged_data, data, on=['Chrom', 'WindowStart', 'WindowEnd'], how='outer')
    # 填充NaN为0
    merged_data = merged_data.fillna(0)
    return merged_data

def process_samples_for_window_size(sample_files, sample_names, fai_file, window_size, output_prefix):
    """处理所有样本并生成特定窗口大小的矩阵"""
    binned_data_list = []
...
def create_correlation_heatmaps(all_correlations, output_prefix):
    ...
    for i, window_size in enumerate(window_sizes):
        for j, pair in enumerate(group_pairs):
            if pair in all_correlations[window_size]:
                ...
                spearman_matrix[i, j] = all_correlations[window_size][pair]['spearman_r']
    ...
    plt.tight_layout()
    plt.savefig(f'{output_prefix}_correlation_heatmaps.png', dpi=300, bbox_inches='tight')
...
def create_correlation_line_plots(all_correlations, output_prefix):
    ...
    for window_size in window_sizes:
        for pair in data_dict.keys():
            if pair in all_correlations[window_size]:
                data_dict[pair]['pearson'].append(all_correlations[window_size][pair]['pearson_r'])
                data_dict[pair]['spearman'].append(all_correlations[window_size][pair]['spearman_r'])
            else:
...
With custom window parameters:
    ...
    # 处理每个组的文件
    for i, (group_files, group_num) in enumerate([(args.group1, 1), (args.group2, 2), (args.group3, 3)]):
    ...
    for window_size in window_sizes:
        ...
        print(f"Sample groups: Group1={len(groups[1])}, Group2={len(groups[2])}, Group3={len(groups[3])}")
        ...
        for pair, stats in correlations.items():
            print(f"  {pair}: Pearson={stats['pearson_r']:.3f} (p={stats['pearson_p']:.3e}), "
    ...
    print(f"  - Correlation heatmaps: {args.output_prefix}_correlation_heatmaps.png/pdf")
