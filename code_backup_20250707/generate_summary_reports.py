...
def analyze_length_distribution(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.dropna(subset=['Length'], inplace=True) # Drop rows where length could not be converted / 删除长度无法转换的行
    ...
    distribution = df.groupby(['sample', 'length_category']).size().unstack(fill_value=0).stack().reset_index(name='Count')
    ...
    distribution['Percentage'] = (distribution['Count'] / distribution['Total'] * 100).round(2)
...
def main():
    ...
    parser.add_argument("-i", "--input-file", required=True, help="Path to the input TSV/CSV file (e.g., Combined_Basic_Results.tsv). / 输入的 TSV/CSV 文件路径。")
    parser.add_argument("-o", "--output-prefix", required=True, help="Prefix for output files (e.g., 'MyProject'). / 输出文件的前缀（例如 'MyProject'）。")
    parser.add_argument("--sep", default='\t', help="Separator for the input file. Default is tab. Use ',' for CSV. / 输入文件的分隔符。默认为制表符，CSV文件请使用','。")
