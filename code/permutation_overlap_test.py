...
class PermutationOverlapTest:
    ...
    def load_input_regions(self) -> Tuple[pd.DataFrame, Path]:
        ...
        if original_count != filtered_count:
            ...
            logger.info(
                f"  Filtered to {filtered_count} regions (removed {', '.join(filters)})"
            )
    ...
    def run_permutation_test(self, annotation_file: Path, annotation_name: str) -> Dict:
        ...
        observed_count = self.run_bedtools_intersect(
            self.input_bed_file, annotation_file
        )
    ...
    def plot_null_distribution(self, results: Dict, output_name: str):
        ...
        plt.axvline(
            results["expected_mean"],
            color="green",
            linestyle=":",
            linewidth=2,
            label=f'Expected ({results["expected_mean"]:.1f})',
        )
    ...
    def plot_summary(self, all_results: List[Dict]):
        ...
        colors = [
            (
                "red"
                if p < 0.001
                else "orange" if p < 0.01 else "yellow" if p < 0.05 else "lightblue"
            )
            for p in p_values
        ]
        ...
        neg_log_p = [
            (
                -np.log10(p)
                if p > 0
                else -np.log10(1 / (all_results[0]["n_permutations"] + 1))
            )
            for p in p_values
        ]
        ...
        ax2.axhline(
            y=-np.log10(0.05), color="gray", linestyle="--", alpha=0.5, label="p=0.05"
        )
        ax2.axhline(
            y=-np.log10(0.01), color="gray", linestyle=":", alpha=0.5, label="p=0.01"
        )
...
"""
        ...
        report_file = (
            f"{self.report_prefix}_report.md"
    ...
    def run(self):
        ...
        try:
            ...
            for i, annotation_file in enumerate(self.annotation_files):
                ...
                results_summary = {
                    k: v for k, v in results.items() if k != "null_distribution"
                }
            ...
            results_file = (
                f"{self.report_prefix}_results.csv"
                if self.report_prefix
                else "permutation_test_results.csv"
            )
...
Examples:
  ...
  %(prog)s -i eccDNA.csv -a enhancers.bed --exclude-sex-chr --report-prefix eccDNA_enhancer -o results/
    ...
    # Required arguments
    parser.add_argument(
        "-i", "--input", required=True, help="Input CSV file with genomic regions"
    )
    parser.add_argument(
        "-a",
        "--annotations",
    ...
    # CSV format options
    parser.add_argument(
        "--chr-col", default="eChr", help="Column name for chromosome (default: eChr)"
    )
    parser.add_argument(
        "--start-col",
        default="eStart",
    ...
    parser.add_argument(
        "--end-col", default="eEnd", help="Column name for end position (default: eEnd)"
    )
    parser.add_argument(
        "--name-col",
        default="eName",
    ...
    parser.add_argument(
        "--no-header", action="store_true", help="Input CSV has no header row"
    )
    ...
    parser.add_argument(
        "--report-prefix", default="", help="Prefix for output files (default: none)"
    )
