...
class eccDNAHotspotAnalyzer:
    ...
    def load_window_data(self, file_10kb, file_50kb, file_100kb):
        ...
        self.samples = [
            col
            for col in self.window_data["10kb"].columns
    ...
    def identify_candidate_hotspots(self, chrom, threshold_factor=2.0):
        ...
        for scale, df in self.window_data.items():
            ...
            signal_cols = [
                col
                for col in chrom_data.columns
                if col not in ["Chrom", "WindowStart", "WindowEnd"]
            ]

            chrom_data["mean_signal"] = chrom_data[signal_cols].mean(axis=1)
            chrom_data["cv"] = chrom_data[signal_cols].std(axis=1) / (
                chrom_data["mean_signal"] + 1e-6
            )
    ...
    def refine_hotspot_boundaries(self, chrom, start, end, resolution=1000):
        ...
        for sample, df in self.raw_eccdna.items():
            ...
            for _, ecc in region_eccdna.iterrows():
                ...
                if start_bin < len(signal_array) and end_bin > 0:
                    signal_array[
                        max(0, start_bin) : min(len(signal_array), end_bin)
                    ] += 1
        ...
        for b_start, b_end in boundaries:
            if b_start <= end and b_end >= start:  # 与原始候选区域重叠
                ...
                if (
                    region_stats["mean_signal"] > 0
                    and region_stats["consistency"] > 0.5
                ):
    ...
    def detect_signal_boundaries(self, signal_array, positions, min_peak_height=0.5):
        ...
        peaks, properties = sp_signal.find_peaks(
            signal_array, height=max(threshold, min_peak_height)
        )
        ...
        for peak in peaks:
            ...
            right_boundary = peak
            while (
                right_boundary < len(signal_array) - 1
                and signal_array[right_boundary] > threshold * 0.5
            ):
                right_boundary += 1
    ...
    def calculate_region_statistics(self, chrom, start, end, signal_tracks, positions):
        ...
        stats["enrichment"] = stats["mean_signal"] / (
            expected / len(self.samples) + 1e-6
        )
        ...
        for chrom in self.chromosomes:
            ...
            for candidate in candidates:
                refined = self.refine_hotspot_boundaries(
                    chrom, candidate["start"], candidate["end"]
                )
            ...
            print(
                f"  精确定义了 {len([h for h in all_hotspots if h['chrom'] == chrom])} 个热点"
            )
    ...
    def save_results(self, hotspots, output_prefix):
        ...
        with open(f"{output_prefix}_hotspots.bed", "w") as f:
            for _, hotspot in df_hotspots.iterrows():
                ...
                f.write(
                    f"{hotspot['chrom']}\t{hotspot['start']}\t{hotspot['end']}\t"
        ...
        with open(f"{output_prefix}_summary.txt", "w") as f:
            ...
            f.write(
                f"核心热点: {len([h for h in hotspots if h['category'] == 'core'])}\n"
            )
            f.write(
                f"复现热点: {len([h for h in hotspots if h['category'] == 'recurrent'])}\n"
            )
            f.write(
                f"可变热点: {len([h for h in hotspots if h['category'] == 'variable'])}\n\n"
            )

            if len(hotspots) > 0:
                f.write(
                    f"平均热点长度: {np.mean([h['length'] for h in hotspots]):.1f} bp\n"
                )
                f.write(f"最小热点长度: {min([h['length'] for h in hotspots])} bp\n")
                f.write(f"最大热点长度: {max([h['length'] for h in hotspots])} bp\n")
                f.write(
                    f"平均富集倍数: {np.mean([h['enrichment'] for h in hotspots]):.2f}\n"
                )

                if "boundary_pvalue" in hotspots[0]:
                    sig_hotspots = [
                        h for h in hotspots if h.get("boundary_pvalue", 1) < 0.05
                    ]
...
示例用法:
  python eccDNA-SHARP.py \\
    ...
    parser.add_argument("--window-10kb", required=True, help="10kb窗口统计文件")
    parser.add_argument("--window-50kb", required=True, help="50kb窗口统计文件")
    parser.add_argument("--window-100kb", required=True, help="100kb窗口统计文件")
    parser.add_argument(
        "--raw-files", nargs="+", required=True, help="原始eccDNA文件列表（CSV格式）"
    )
    ...
    parser.add_argument(
        "--permutation-test", action="store_true", help="对热点边界进行置换检验"
    )
    parser.add_argument(
        "--n-permutations", type=int, default=1000, help="置换检验次数（默认: 1000）"
    )
    ...
    # 加载数据
    analyzer.load_window_data(args.window_10kb, args.window_50kb, args.window_100kb)
    analyzer.load_raw_eccdna(args.raw_files)

    # 执行分析
    hotspots = analyzer.analyze(
        args.output_prefix, do_permutation=args.permutation_test
    )
