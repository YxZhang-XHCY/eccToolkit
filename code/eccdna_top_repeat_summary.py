...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze eccDNA repeat motifs by species and class"
    )
    parser.add_argument("-i", "--input", required=True, help="Input CSV file")
    parser.add_argument(
        "-o", "--output", required=True, help="Output directory for result files"
    )
    ...
    try:
        ...
        for species in species_list:
            ...
            for class_name in classes_list:
                ...
                print(f"\n{'='*60}")
                print(
                    f"Processing: {species} - {class_name} ({len(species_class_df)} rows)"
                )
                print(f"{'='*60}")
                ...
                if len(result_df) > 0:
                    ...
                    output_file = os.path.join(
                        output_dir, f"{species}_{class_name}_motif_summary.csv"
                    )
