...
class eccDNAHotspotDetector:
    ...
    def permutation_pval_for_hotspot_worker(args):
        ...
        dfc['in_hotspot'] = (dfc['WindowStart'] < hotspot_row['end']) & (dfc['WindowEnd'] > hotspot_row['start'])
    ...
    def run_analysis(self, file_paths, n_permutations=1000, n_jobs=8):
        ...
        for idx, row in hotspots_df.iterrows():
            ...
            args_list.append((row, win_df, sample_cols, n_permutations, idx + 100))  # Different seed per job
    ...
    def generate_report(self, hotspots_df, output_prefix):
        ...
        if not core_df.empty:
            print(core_df[['chrom', 'start', 'end', 'mean_signal', 'mean_fold', 'pval', 'qval']].head(10).to_string(index=False))
        else:
            print("No core hotspots found.")
        print("\n===== SIGNIFICANT CORE HOTSPOTS (FDR<0.05) =====")
        if not sig_core_df.empty:
            print(sig_core_df[['chrom', 'start', 'end', 'mean_signal', 'mean_fold', 'pval', 'qval']].head(10).to_string(index=False))
        else:
...
def main():
    parser = argparse.ArgumentParser(description='Detect eccDNA hotspots from multi-scale data with parallel permutation FDR test and tqdm progress')
