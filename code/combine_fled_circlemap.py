...
def process_fled_output(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        logging.info(
            f"  Found {len(df_filtered):,} valid records in sample {df_filtered['sample'].iloc[0]}"
        )
        return df_filtered
...
def process_circlemap_output(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        columns = [
            ...
            "Mean_coverage",
            "Standard_deviation",
            "Coverage_increase_start",
            "Coverage_increase_end",
            "Coverage_continuity",
        ]
        df = pd.read_csv(file_path, sep="\t", header=None, names=columns)
...
def main() -> None:
    ...
    parser.add_argument(
        "--fled-dir",
        required=True,
        help="Directory containing FLED output files. / 包含 FLED 输出文件的目录。",
    )
    parser.add_argument(
        "--circlemap-dir",
        required=True,
        help="Directory containing Circle-Map output files. / 包含 Circle-Map 输出文件的目录。",
    )
    parser.add_argument(
        "--fled-pattern",
        default="*.DiGraph.OnesegJunction.out",
        help="Filename pattern for FLED files. / 用于查找 FLED 文件的文件名模式。",
    )
    parser.add_argument(
        "--circlemap-pattern",
        default="*_CM.bed",
        help="Filename pattern for Circle-Map files. / 用于查找 Circle-Map 文件的文件名模式。",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory to save the final result files. / 用于保存最终结果文件的目录。",
    )
    ...
    fled_files = glob.glob(
        os.path.join(args.fled_dir, args.fled_pattern)
    )  # Find all files matching the FLED pattern / 查找所有匹配 FLED 模式的文件
    if not fled_files:
        logging.warning(
            f"No FLED files found in '{args.fled_dir}' with pattern '{args.fled_pattern}'"
        )
        fled_df = pd.DataFrame()
    else:
        fled_df = pd.concat(
            [process_fled_output(f) for f in fled_files], ignore_index=True
        )

    # --- Process Circle-Map files / 处理 Circle-Map 文件 ---
    circlemap_files = glob.glob(
        os.path.join(args.circlemap_dir, args.circlemap_pattern)
    )  # Find all files matching the Circle-Map pattern / 查找所有匹配 Circle-Map 模式的文件
    if not circlemap_files:
        logging.warning(
            f"No Circle-Map files found in '{args.circlemap_dir}' with pattern '{args.circlemap_pattern}'"
        )
        circlemap_df = pd.DataFrame()
    else:
        circlemap_df = pd.concat(
            [process_circlemap_output(f) for f in circlemap_files], ignore_index=True
        )
    ...
    if not fled_df.empty:
        fled_simple = fled_df[
            ["name", "sample", "Length", "chrom", "start", "end"]
    ...
    if not circlemap_df.empty:
        circlemap_simple = circlemap_df[
            ["name", "sample", "Length", "Chromosome", "Start", "End"]
        ]
        circlemap_simple["Source"] = "Circle-Map"

    combined_df = pd.concat(
        [fled_simple, circlemap_simple], ignore_index=True
    ...
    if not fled_df.empty:
        ...
        fled_df_final.to_csv(
            os.path.join(args.output_dir, "FLED_filtered_results.csv"), index=False
        )
        logging.info(f"FLED results saved. Total records: {len(fled_df_final):,}")

    if not circlemap_df.empty:
        other_circlemap_cols = [
            c for c in circlemap_df.columns if c not in primary_cols[:3]
        ]
        circlemap_df_final = circlemap_df[primary_cols[:3] + other_circlemap_cols]
        circlemap_df_final.to_csv(
            os.path.join(args.output_dir, "CircleMap_filtered_results.csv"),
            index=False,
            float_format="%.2f",
        )
        logging.info(
            f"Circle-Map results saved. Total records: {len(circlemap_df_final):,}"
        )

    if not combined_df.empty:
        combined_df.to_csv(
            os.path.join(args.output_dir, "FLED_and_CircleMap_combined.csv"),
            index=False,
        )
        logging.info(f"Combined results saved. Total records: {len(combined_df):,}")

    logging.info(
        f"✔ Processing complete! All output files are saved in: {args.output_dir}"
    )
