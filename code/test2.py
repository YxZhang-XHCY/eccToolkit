...
def analyze_eccdna_degs_overlap(eccdna_df, degs_df, sample_name=None, fc_threshold=1):
    ...
    # 获取显著的eccDNA基因
    eccdna_enriched = set(eccdna_sample[(eccdna_sample['direction'] == 'enrichment') & 
                                        (eccdna_sample['significant'] == True)]['gene'])
    eccdna_depleted = set(eccdna_sample[(eccdna_sample['direction'] == 'depletion') & 
                                        (eccdna_sample['significant'] == True)]['gene'])
...
def perform_gradient_fc_analysis(eccdna_df, degs_df, fc_thresholds=[1, 2, 4, 6]):
    ...
    eccdna_enriched_all = set(eccdna_df[(eccdna_df['direction'] == 'enrichment') & 
                                        (eccdna_df['significant'] == True)]['gene'])
    eccdna_depleted_all = set(eccdna_df[(eccdna_df['direction'] == 'depletion') & 
                                        (eccdna_df['significant'] == True)]['gene'])
    ...
    for fc in fc_thresholds:
        ...
        # 分析当前FC阈值
        results, stats, _, _, _, _ = analyze_eccdna_degs_overlap(eccdna_df, degs_df, fc_threshold=fc)
        ...
        if stats['n_eccdna_enriched'] > 0 and stats['n_degs_up'] > 0:
            ...
            print(f"  Enriched ∩ Up: n={stats['n_enriched_up']}, OR={or_eu:.3f}, p={p_eu:.3e}")
        ...
        if stats['n_eccdna_depleted'] > 0 and stats['n_degs_down'] > 0:
            ...
            print(f"  Depleted ∩ Down: n={stats['n_depleted_down']}, OR={or_dd:.3f}, p={p_dd:.3e}")
        
        # Fisher检验：enriched vs down (反向)
        if stats['n_eccdna_enriched'] > 0 and stats['n_degs_down'] > 0:
            or_ed, p_ed = fisher_test(stats['n_enriched_down'], stats['n_eccdna_enriched'], 
                                     stats['n_degs_down'], total_genes)
            analysis_results['enriched_down_OR'] = or_ed
            analysis_results['enriched_down_pvalue'] = p_ed
            print(f"  Enriched ∩ Down: n={stats['n_enriched_down']}, OR={or_ed:.3f}, p={p_ed:.3e}")
        
        # Fisher检验：depleted vs up (反向)
        if stats['n_eccdna_depleted'] > 0 and stats['n_degs_up'] > 0:
            or_du, p_du = fisher_test(stats['n_depleted_up'], stats['n_eccdna_depleted'], 
                                     stats['n_degs_up'], total_genes)
            analysis_results['depleted_up_OR'] = or_du
            analysis_results['depleted_up_pvalue'] = p_du
            print(f"  Depleted ∩ Up: n={stats['n_depleted_up']}, OR={or_du:.3f}, p={p_du:.3e}")
...
def plot_gradient_analysis(gradient_df):
    ...
    if 'enriched_up_OR' in gradient_df.columns:
        ax.plot(gradient_df['fc_threshold'], gradient_df['enriched_up_OR'], 'o-', label='Enriched-Up', color='red')
    if 'depleted_down_OR' in gradient_df.columns:
        ax.plot(gradient_df['fc_threshold'], gradient_df['depleted_down_OR'], 's-', label='Depleted-Down', color='blue')
    if 'enriched_down_OR' in gradient_df.columns:
        ax.plot(gradient_df['fc_threshold'], gradient_df['enriched_down_OR'], '^-', label='Enriched-Down', color='orange')
    if 'depleted_up_OR' in gradient_df.columns:
        ax.plot(gradient_df['fc_threshold'], gradient_df['depleted_up_OR'], 'v-', label='Depleted-Up', color='green')
    ...
    if 'enriched_up_pvalue' in gradient_df.columns:
        ax.plot(gradient_df['fc_threshold'], -np.log10(gradient_df['enriched_up_pvalue']), 'o-', label='Enriched-Up', color='red')
    if 'depleted_down_pvalue' in gradient_df.columns:
        ax.plot(gradient_df['fc_threshold'], -np.log10(gradient_df['depleted_down_pvalue']), 's-', label='Depleted-Down', color='blue')
...
def plot_fc_specific_venn(eccdna_df, degs_df, fc_threshold=2):
    ...
    results, stats, eccdna_enriched, eccdna_depleted, degs_up, degs_down = analyze_eccdna_degs_overlap(
        eccdna_df, degs_df, fc_threshold=fc_threshold)
    
    fig = plot_venn_diagrams(eccdna_enriched, eccdna_depleted, degs_up, degs_down, 
                            f"(FC threshold: {fc_threshold})")
    return fig

def plot_venn_diagrams(eccdna_enriched, eccdna_depleted, degs_up, degs_down, sample_name=""):
    """绘制韦恩图"""
    ...
    ax = axes[0, 0]
    venn2([eccdna_enriched, degs_up], 
          set_labels=['eccDNA Enriched', 'RNA-seq Up'], ax=ax)
    ax.set_title('eccDNA Enriched vs RNA-seq Upregulated')
    
    # 2. eccDNA耗竭 vs RNA-seq下调
    ax = axes[0, 1]
    venn2([eccdna_depleted, degs_down], 
          set_labels=['eccDNA Depleted', 'RNA-seq Down'], ax=ax)
    ax.set_title('eccDNA Depleted vs RNA-seq Downregulated')
    
    # 3. eccDNA富集 vs RNA-seq下调（反向）
    ax = axes[1, 0]
    venn2([eccdna_enriched, degs_down], 
          set_labels=['eccDNA Enriched', 'RNA-seq Down'], ax=ax)
    ax.set_title('eccDNA Enriched vs RNA-seq Downregulated (Inverse)')
    
    # 4. eccDNA耗竭 vs RNA-seq上调（反向）
    ax = axes[1, 1]
    venn2([eccdna_depleted, degs_up], 
          set_labels=['eccDNA Depleted', 'RNA-seq Up'], ax=ax)
...
def analyze_sample_overlap(eccdna_df):
    ...
    for sample in samples:
        ...
        sample_enriched[sample] = set(sample_data[(sample_data['direction'] == 'enrichment') & 
                                                  (sample_data['significant'] == True)]['gene'])
        sample_depleted[sample] = set(sample_data[(sample_data['direction'] == 'depletion') & 
                                                  (sample_data['significant'] == True)]['gene'])
...
def plot_sample_overlap_venn(sample_genes, title, max_samples=3):
    ...
    if len(samples) == 2:
        ...
        venn2([sample_genes[samples[0]], sample_genes[samples[1]]], 
              set_labels=samples, ax=ax)
        ax.set_title(title)
    elif len(samples) >= 3:
        fig, ax = plt.subplots(figsize=(8, 6))
        venn3([sample_genes[samples[0]], sample_genes[samples[1]], sample_genes[samples[2]]], 
              set_labels=samples[:3], ax=ax)
        ax.set_title(title)
...
def create_overlap_heatmap(eccdna_df, degs_df):
    ...
    for sample in samples:
        sample_data = eccdna_df[eccdna_df['Sample'] == sample]
        enriched = sample_data[(sample_data['direction'] == 'enrichment') & 
                              (sample_data['significant'] == True)]['gene']
        depleted = sample_data[(sample_data['direction'] == 'depletion') & 
                              (sample_data['significant'] == True)]['gene']
...
def perform_chi_square_trend_test(gradient_results):
    ...
    for assoc in associations:
        ...
        if den > 0:
            ...
            print(f"{assoc}: slope={slope:.3f}, χ²={chi2_stat:.3f}, p={p_value:.3e}, trend={trend_results[assoc]['trend']}")
...
def calculate_enrichment_score(eccdna_df, degs_df):
    """计算每个基因的综合富集评分"""
    # 合并eccDNA数据（取平均值）
    eccdna_summary = eccdna_df[eccdna_df['significant'] == True].groupby('gene').agg({
        'fold_change': 'mean',
        'z_score': 'mean',
    ...
    # 合并DEG数据
    merged = pd.merge(eccdna_summary, degs_df[['gene', 'log2fc', 'qvalue']], 
                     on='gene', how='outer')
    
    # 计算综合评分
    merged['eccdna_score'] = merged['fold_change'].fillna(1) * np.sign(merged['z_score'].fillna(0))
...
def plot_correlation_analysis(eccdna_df, degs_df):
    ...
    for direction, color in colors.items():
        ...
        ax.scatter(subset['log2fc'], np.log2(subset['fold_change']), 
                  c=color, alpha=0.6, label=f'eccDNA {direction}', s=50)
    
    # 添加相关性
    if len(gene_scores) > 3:
        corr, p_val = spearmanr(gene_scores['log2fc'], np.log2(gene_scores['fold_change']))
    ...
    colors_cat = {'Concordant_positive': 'darkgreen',
    ...
    quadrant_names = ['Q1: Up/Enriched', 'Q2: Down/Enriched', 
                      'Q3: Down/Depleted', 'Q4: Up/Depleted']
    quadrant_counts = [quadrants[q].sum() for q in ['Q1', 'Q2', 'Q3', 'Q4']]
    
    ax.pie(quadrant_counts, labels=quadrant_names, autopct='%1.1f%%',
...
def perform_pathway_enrichment_prep(gene_scores):
    ...
    # 创建不同类别的基因列表
    gene_lists = {
        'concordant_positive': gene_scores[gene_scores['category'] == 'Concordant_positive']['gene'].tolist(),
        'concordant_negative': gene_scores[gene_scores['category'] == 'Concordant_negative']['gene'].tolist(),
        'discordant': gene_scores[gene_scores['category'] == 'Discordant']['gene'].tolist(),
        'eccdna_enriched_only': gene_scores[(gene_scores['direction'] == 'enrichment') & 
                                           (pd.isna(gene_scores['log2fc']))]['gene'].tolist(),
        'eccdna_depleted_only': gene_scores[(gene_scores['direction'] == 'depletion') & 
                                           (pd.isna(gene_scores['log2fc']))]['gene'].tolist(),
        'deg_up_only': gene_scores[(gene_scores['log2fc'] > 0) & 
                                   (pd.isna(gene_scores['fold_change']))]['gene'].tolist(),
        'deg_down_only': gene_scores[(gene_scores['log2fc'] < 0) & 
                                     (pd.isna(gene_scores['fold_change']))]['gene'].tolist()
    }
...
def main():
    ...
    results, stats, eccdna_enriched, eccdna_depleted, degs_up, degs_down = analyze_eccdna_degs_overlap(
        eccdna_df, degs_df, fc_threshold=1)
    ...
    # 5. 绘制样本间韦恩图
    if len(samples) >= 2:
        fig2 = plot_sample_overlap_venn(sample_enriched, "eccDNA Enriched Genes Overlap", 3)
        plt.savefig('eccdna_enriched_sample_venn.png', dpi=300, bbox_inches='tight')
        
        fig3 = plot_sample_overlap_venn(sample_depleted, "eccDNA Depleted Genes Overlap", 3)
    ...
    if len(significant_genes) > 0:
        ...
        sns.heatmap(significant_genes.T, cmap='RdBu_r', center=0, 
    ...
    # 合并数据以创建散点图
    eccdna_gene_stats = eccdna_df[eccdna_df['significant'] == True].groupby('gene').agg({
        'fold_change': 'mean',
        'direction': 'first'
    }).reset_index()
    
    # 合并DEG数据
    merged_data = pd.merge(eccdna_gene_stats, degs_df[['gene', 'log2fc']], on='gene', how='inner')
    ...
    for direction, color in colors.items():
        ...
        plt.scatter(subset['log2fc'], subset['fold_change'], 
                   c=color, alpha=0.6, label=f'eccDNA {direction}')
    ...
    with open('comprehensive_analysis_report.txt', 'w') as f:
        ...
        for assoc, results in trend_results.items():
            f.write(f"{assoc}: {results['trend']} trend, χ²={results['chi2']:.3f}, p={results['p_value']:.3e}\n")
        f.write("\n")
        ...
        concordant_pos = gene_scores[gene_scores['category'] == 'Concordant_positive'].nlargest(20, 'combined_score')
        for _, row in concordant_pos.iterrows():
            f.write(f"  {row['gene']}: eccDNA_FC={row['fold_change']:.2f}, RNA_log2FC={row['log2fc']:.2f}\n")
        
        # 负向一致的基因
        f.write("\nTop Concordant Negative (eccDNA depleted + RNA downregulated):\n")
        concordant_neg = gene_scores[gene_scores['category'] == 'Concordant_negative'].nsmallest(20, 'combined_score')
