...
def calculate_eccdna_composition(df):
    ...
    try:
        ...
        # 使用更高效的聚合方式
        eccdna_summary = (
            df_sorted.groupby(["Sample", "Class", "seqname"])
            .agg(
                {
                    "anno_Percent": "sum",
                    "motif": lambda x: "|".join(
                        [str(m) for m in x if pd.notna(m)]
                    ),  # 保留所有motif，已按位置排序
                    "seq_length": "first",
                }
            )
        ...
        eccdna_summary["motif_combination_by_count"] = eccdna_summary[
            "motif_combination"
        ].apply(create_count_combination)
        eccdna_summary["transposon_count"] = eccdna_summary["motif_combination"].apply(
            count_transposons
        )
        eccdna_summary["unique_transposon_types"] = eccdna_summary[
            "motif_combination"
        ].apply(count_unique_transposon_types)
        ...
        print(
            df[
                ["Sample", "Class", "seqname", "anno_Percent", "motif", "seq_length"]
            ].head(10)
        )
        raise
...
def filter_high_coverage_eccdna(eccdna_summary, threshold=60):
    ...
    filtered_df = eccdna_summary[
        eccdna_summary["total_anno_percent"] > threshold
    ].copy()

    print(
        f"筛选后eccDNA数量: {len(filtered_df)} ({len(filtered_df)/len(eccdna_summary)*100:.1f}%)"
    )

    # 按样本和类型统计
    sample_class_counts = (
        filtered_df.groupby(["Sample", "Class"]).size().reset_index(name="count")
    )
    print("\n各样本各类型的高覆盖eccDNA数量:")
...
def generate_composition_report(filtered_df, results):
    ...
    # 分析组合复杂度（从位置排序组合）
    combo_complexity_pos = [
        len(combo.split("|")) for combo in filtered_df["motif_combination"]
    ]
    print(f"\nmotif组合复杂度分析（按位置）:")
    ...
    for num_motifs, count in sorted(complexity_dist_pos.items()):
        print(
            f"    {num_motifs}个motif: {count} eccDNA ({count/len(combo_complexity_pos)*100:.1f}%)"
        )
    ...
    for num_types, count in sorted(unique_types_dist.items()):
        print(
            f"    {num_types}种独特motif: {count} eccDNA ({count/len(unique_motif_types)*100:.1f}%)"
        )
    ...
    print(
        f"  包含单个转座子的eccDNA数量: {(filtered_df['transposon_count'] == 1).sum()}"
    )
    print(
        f"  包含多个转座子的eccDNA数量: {(filtered_df['transposon_count'] > 1).sum()}"
    )
...
def save_results(filtered_df, output_file="high_coverage_eccdna.csv"):
    ...
    columns_order = [
        ...
        "total_anno_percent",
        "transposon_count",
        "unique_transposon_types",
        "motif_combination",
        "motif_combination_by_count",
        "seq_length",
    ]
...
def create_visualizations(filtered_df, results):
    ...
    sns.barplot(
        data=sample_class_counts, x="Sample", y="count", hue="Class", ax=axes[0, 0]
    )
    ...
    # 2. anno_Percent分布
    axes[0, 1].hist(
        filtered_df["total_anno_percent"], bins=20, alpha=0.7, edgecolor="black"
    )
    ...
    # 5. 序列长度分布
    axes[1, 1].scatter(
        filtered_df["seq_length"],
        filtered_df["total_anno_percent"],
        c=filtered_df["transposon_count"],
        cmap="viridis",
        alpha=0.6,
    )
    ...
    axes[1, 2].scatter(
        filtered_df["transposon_count"], filtered_df["total_anno_percent"], alpha=0.6
    )
...
def main():
    ...
    try:
        ...
        # 4. 分析motif组合
        results, overall_combinations, overall_combinations_count = (
            analyze_motif_combinations(filtered_df)
        )
