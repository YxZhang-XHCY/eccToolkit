...
def analyze_anno_percent_by_class(df, te_class):
    ...
    # By Sample
    sample_stats = (
        df_class.groupby(["Sample", "anno_bin"], observed=True)
        .size()
...
def plot_anno_percent(df_class, sample_percent, te_class):
    ...
    fig.suptitle(
        f"anno_Percent Distribution - {te_class}", fontsize=15, fontweight="bold"
    )

    # Histogram
    axes[0].hist(
        df_class["anno_Percent"], bins=50, alpha=0.7, color="orange", edgecolor="black"
    )
    axes[0].set_title("anno_Percent Histogram")
    axes[0].set_xlabel("anno_Percent (%)")
    axes[0].set_ylabel("Frequency")
    axes[0].axvline(
        df_class["anno_Percent"].mean(),
        color="red",
        linestyle="--",
        label=f'Mean: {df_class["anno_Percent"].mean():.1f}%',
    )
    axes[0].legend()
    ...
    bin_counts = df_class["anno_bin"].value_counts().sort_index()
    axes[1].bar(
        range(len(bin_counts)), bin_counts.values, color="salmon", edgecolor="black"
    )
