...
def get_args():
    ...
    p.add_argument("-i", "--tsv", required=True, help="Input TSV file with eccDNA candidates (Chr, Start, End, Name). / 包含候选eccDNA的TSV输入文件。")
    p.add_argument("-1", "--fastq1", required=True, help="Input FASTQ file, read 1 (gzipped OK). / 输入的FASTQ文件，Read 1。")
    p.add_argument("-2", "--fastq2", help="Input FASTQ file, read 2 (optional, for paired-end). / 输入的FASTQ文件，Read 2 (可选，用于双端测序)。")
    p.add_argument("-r", "--ref", required=True, help="Reference genome FASTA file (must be indexed). / 参考基因组FASTA文件(必须已建立索引)。")
    p.add_argument("-o", "--prefix", default="eccdna_validation", help="Prefix for all output files. / 所有输出文件的前缀。")
    p.add_argument("-t", "--threads", type=int, default=8, help="Number of threads for BWA and samtools. / BWA和samtools使用的线程数。")
    
    # Method-specific parameters / 方法相关参数
    p.add_argument("--inside", type=int, default=300, help="Length (bp) to keep from the inside of each eccDNA end. / 从eccDNA每个末端内侧保留的序列长度(bp)。")
    p.add_argument("--flank", type=int, default=200, help="Length (bp) to keep from the flanking region of each eccDNA end. / 从eccDNA每个末端外侧(旁翼)保留的序列长度(bp)。")
    
    # Analysis thresholds / 分析阈值参数
    p.add_argument("--window", type=int, default=20, help="Half-window size around the junction center to define a crossing read. / 定义跨越reads时，环绕连接点中心点的半窗口大小。")
    p.add_argument("--min-crossing", type=int, default=3, help="Minimum number of crossing reads to support a junction. / 支持一个连接点所需的最小跨越reads数。")
    p.add_argument("--min-unique", type=int, default=20, help="Minimum number of unique reads mapped to the amplicon. / 比对到扩增子上的最小唯一reads数。")
    p.add_argument("--min-split", type=int, default=3, help="Minimum number of split reads mapped to the amplicon. / 比对到扩增子上的最小剪切reads数。")
    return p.parse_args()
...
def build_amplicon_fasta(tsv_path: str, ref_fa_path: str, out_fa_path: str, inside_len: int, flank_len: int):
    """
    ...
    with open(tsv_path) as fh, open(out_fa_path, "w") as out:
        ...
        for row in reader:
            ...
            seq = (ref.fetch(chrom, tail_in_s - 1, tail_in_e) +    # Tail-Inside / 环内尾部
                   ref.fetch(chrom, tail_out_s - 1, tail_out_e) +  # Tail-Outside / 环外尾部
                   ref.fetch(chrom, head_out_s - 1, head_out_e) +  # Head-Outside / 环外头部
                   ref.fetch(chrom, head_in_s - 1, head_in_e))     # Head-Inside / 环内头部
...
def bwa_align(fa_path: str, fq1_path: str, fq2_path: str, threads: int, prefix: str) -> str:
    """
    ...
    bwa_cmd = f"bwa mem -t {threads} {fa_path} {fq1_path}" + (f" {fq2_path}" if fq2_path else "")
...
def sa_tag_crosses(aln: pysam.AlignedSegment, center: int, window: int) -> bool:
    ...
    for block in aln.get_tag("SA").split(';'):
        if not block: continue # Handle trailing semicolon / 处理末尾的分号
        
        sa_pos = int(block.split(',')[1]) - 1  # SA tag position is 1-based / SA标签的位置是1-based
...
def determine_status(unique_reads, split_reads, crossing_reads, thresholds):
    ...
    basic_pass = unique_reads >= thresholds.min_unique and split_reads >= thresholds.min_split
...
def analyze_bam(bam_path: str, args: argparse.Namespace) -> pd.DataFrame:
    ...
    for ref_name in bam.references:
        ...
        for aln in bam.fetch(ref_name):
            ...
            if crosses_center(aln, center, args.window) or sa_tag_crosses(aln, center, args.window):
                crossing_reads.add(aln.query_name)
        
        # Determine status and confidence / 判定状态和置信度
        status, confidence = determine_status(len(unique_reads), split_read_count, len(crossing_reads), args)
        ...
        print(f"  {ref_name:<20}\tUnique: {len(unique_reads):<5}\tSplit: {split_read_count:<5}\tCrossing: {len(crossing_reads):<5}\tStatus: {status} ({confidence})", file=sys.stderr)
...
def main():
    ...
    print(f"   - Custom Reference: inside={args.inside}bp, flank={args.flank}bp", file=sys.stderr)
    print(f"   - Detection Thresholds: unique≥{args.min_unique}, split≥{args.min_split}, crossing≥{args.min_crossing}", file=sys.stderr)
    ...
    try:
        ...
        summary = df_results[['ecc_id', 'status', 'confidence', 'unique_reads', 'crossing_reads']].copy()
        ...
        if n_total > 0:
            ...
            print(f"   Detected: {n_detected} ({n_detected/n_total*100:.1f}%)", file=sys.stderr)
            print(f"     - High Confidence: {n_high}", file=sys.stderr)
            print(f"     - Medium Confidence: {n_medium}", file=sys.stderr)
        else:
            print(f"\n## Validation Complete! No candidates were processed.", file=sys.stderr)
