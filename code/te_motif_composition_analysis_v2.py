...
def analyze_motif_composition(csvfile):
    ...
    motif_per_seq["type"] = motif_per_seq["motif_count"].apply(
        lambda x: "single" if x == 1 else "composite"
    )

    # 合并标记结果回原表
    merged = df.merge(
        motif_per_seq[group_cols + ["motif_count", "type"]], on=group_cols
    )
    ...
    print(
        "已输出单一转座子文件 single_motif_eccDNA.csv 和复合转座子文件 composite_motif_eccDNA.csv"
    )

    # ---- motif_percent 分析 ----
    if "motif_percent" in single_rows.columns:
        motif_percent = pd.to_numeric(
            single_rows["motif_percent"], errors="coerce"
        ).dropna()
        desc = motif_percent.describe(percentiles=[0.05, 0.25, 0.5, 0.75, 0.95])
        desc.to_csv("single_motif_percent_stats.csv")
        print(
            "已输出单一转座子的motif_percent描述性统计 single_motif_percent_stats.csv"
        )
        # 可选：画分布图
    ...
    for (sample, class_), group in motif_per_seq.groupby(["Sample", "Class"]):
        ...
        single_motifs = [
            motif_list[0]
            for motif_list in group.loc[group["type"] == "single", "motif"]
        ]
        ...
        mean_motif_per_composite = (
            composite_counts.mean() if len(composite_counts) else 0
        )
    ...
    with open("motif_composition_detail.txt", "w") as f:
        for r in results:
            ...
            f.write(
                f"  复合转座子数量: {r['composite_count']} ({r['composite_ratio']})\n"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="分析eccDNA中单一/复合转座子的比例及motif组合"
    )
