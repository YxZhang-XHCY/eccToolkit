...
def analyze_single_sample(
    df_sample, cutoff, period_range, long_tail_min_len, tolerance=5
):
    """
    ...
    trend = lowess(smoothed_signal, np.arange(len(smoothed_signal)), frac=0.1, it=0)[
        :, 1
    ]
    ...
    try:
        ...
        acf_peaks_indices, acf_props = find_peaks(
            acf_vals[min_lag:], prominence=0.01, height=0.01, distance=5
        )
        acf_peaks = acf_peaks_indices + min_lag
        acf_heights = acf_vals[min_lag:][acf_peaks_indices]
        acf_prominences = acf_props.get(
            "prominences", np.zeros_like(acf_peaks, dtype=float)
        )
    except Exception:
        acf_peaks, acf_heights, acf_prominences = (
            np.array([]),
            np.array([]),
            np.array([]),
        )
    ...
    try:
        ...
        fft_peaks_indices, _ = find_peaks(
            ...
            prominence=(
                np.quantile(fft_power[valid_indices], 0.8) if any(valid_indices) else 1
            ),
            height=(
                np.quantile(fft_power[valid_indices], 0.75) if any(valid_indices) else 1
            ),
            distance=5,
        )
    ...
    if len(long_tail_data) > 1:
        long_tail_stats = {
            ...
            "tail_abundance_ratio": (
                len(long_tail_data) / total_eccdnas if total_eccdnas > 0 else 0
            ),
        }
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Quantify eccDNA Distributions (v5 - Data-Driven Bins)."
    )
    parser.add_argument("--input", required=True, help="Path to the input CSV file.")
    parser.add_argument("--output", required=True, help="Path to the output directory.")
    parser.add_argument(
        "--cutoffs", default="2000", help="Periodicity analysis length cutoff."
    )
    parser.add_argument(
        "--long_tail_thresholds",
        default="5000",
        help="Thresholds for defining the long tail.",
    )
    parser.add_argument(
        "--min_records",
        type=int,
        default=1000,
        help="Minimum number of records per sample.",
    )
    ...
    progress_bar = tqdm(
        analysis_combinations, desc="Quantifying & Binning Fingerprints"
    )

    for sample_name, cutoff, long_tail_thresh in progress_bar:
        df_sample = df[df["sample"] == sample_name]
        if len(df_sample) < args.min_records:
            continue

        try:
            results = analyze_single_sample(
                df_sample, cutoff, (5, 500), long_tail_thresh
            )

            summary_row = {
                "sample": sample_name,
                "cutoff": cutoff,
                "long_tail_threshold": long_tail_thresh,
                "noise_strength_200bp": results["noise_strength_200bp"],
                **results["long_tail_stats"],
            }
            ...
            for bin_label, total_intensity in binned_intensities.items():
                if total_intensity > 0:
                    period_results_long_format.append(
                        {
                            "sample": sample_name,
                            "period_bin": bin_label,
    ...
    if long_tail_results:
        pd.DataFrame(long_tail_results).to_csv(
            os.path.join(args.output, "all_samples_multithresh_summary.csv"),
            index=False,
        )
    ...
    if period_results_long_format:
        ...
        binned_matrix = period_df.pivot_table(
            index="sample", columns="period_bin", values="intensity", fill_value=0
        )
        ...
        matrix_filename = os.path.join(
            args.output, "binned_period_intensity_matrix.csv"
        )
