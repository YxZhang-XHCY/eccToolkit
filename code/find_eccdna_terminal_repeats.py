...
def check_dependencies():
    ...
    for tool in tools:
        if shutil.which(tool) is None:
            logging.error(
                f"FATAL: Required tool '{tool}' not found in PATH. Please install it."
            )
            all_found = False
    if not all_found:
        sys.exit(1)
...
def run_command(cmd, log_file=None):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if any(
                msg.lower() in (result.stderr + result.stdout).lower()
                for msg in no_hits_messages
            ):
...
class BlastAnalyzer(BaseProcessor):
    ...
    def _load_fai(self):
        ...
        if not self.fai_file or not self.fai_file.exists():
            logging.warning(
                f"FAI file not provided or does not exist: {self.fai_file}. Chromosome boundary checks will be limited."
            )
        ...
        try:
            ...
            logging.info(
                f"Loaded {len(chrom_lengths)} chromosome lengths from {self.fai_file}"
            )
        except Exception as e:
    ...
    def _clamp_coords(self, chrom, start, end):
        ...
        chrom_len = (
            self.chrom_lengths.get(chrom)
            or self.chrom_lengths.get(f"chr{chrom}")
            or self.chrom_lengths.get(chrom.replace("chr", ""))
        )
        if chrom_len:
    ...
    def _get_fasta(self, chrom, start, end, ecc_name, suffix, temp_dir):
        ...
        if not is_valid:
            logging.warning(
                f"Skipping FASTA extraction for {ecc_name} ({suffix}): region {chrom}:{start}-{end} is invalid after clamping."
            )
            return None
    ...
    def _run_blast(self, query, subject, ecc_name, temp_dir):
        ...
        cmd = [
            ...
            str(blast_out),
            "-outfmt",
            "6 std qseq qlen sseq slen",
            "-task",
            self.blast_params["task"],
            "-word_size",
            str(self.blast_params["word_size"]),
            "-evalue",
            str(self.blast_params["evalue"]),
        ]
        success, _ = run_command(cmd)
        ...
        try:
            ...
            df_filtered = df[
                (df["pident"] >= self.blast_params["min_identity"])
                & (df["length"] >= self.blast_params["min_length"])
            ].copy()
    ...
    def process(self, ecc_data_tuple):  # Renamed from process_single_eccdna for clarity
        ...
        process_temp_dir = Path(
            tempfile.mkdtemp(prefix=f"ecc_{ecc_name}_", dir=self.parent_temp_dir)
        )

        try:
            # Get flank sequences / 获取旁翼序列
            start_flank_fasta = self._get_fasta(
                ecc_chr,
                ecc_start - self.extend_outer,
                ecc_start + self.extend_inner,
                ecc_name,
                "start_flank",
                process_temp_dir,
            )
            end_flank_fasta = self._get_fasta(
                ecc_chr,
                ecc_end - self.extend_inner,
                ecc_end + self.extend_outer,
                ecc_name,
                "end_flank",
                process_temp_dir,
            )
            ...
            direct_df = self._run_blast(
                start_flank_fasta, end_flank_fasta, ecc_name, process_temp_dir
            )

            # --- Inverted Repeats Analysis / 反向重复分析 ---
            reversed_end_fasta = (
                process_temp_dir / f"{end_flank_fasta.stem}_revcomp.fasta"
            )
            if self._reverse_complement_fasta(end_flank_fasta, reversed_end_fasta):
                inverted_df = self._run_blast(
                    start_flank_fasta, reversed_end_fasta, ecc_name, process_temp_dir
                )
            else:
...
class RepeatResultsProcessor(BaseProcessor):
    ...
    def process(self):
        ...
        df["q_genome_start"] = (
            pd.to_numeric(q_coords[1], errors="coerce") + df["qstart"]
        )
        ...
        df.dropna(
            subset=["q_genome_start", "s_genome_start", "ecc_start", "ecc_end"],
            inplace=True,
        )
        if df.empty:
            self.output_path.touch()
        ...
        logging.info(
            f"Processed {len(self.df)} BLAST hits -> {len(df_final)} terminal repeats (is_inverted={self.is_inverted})."
        )
...
class MainProcessor(BaseProcessor):
    ...
    def __init__(self, args):
        ...
        self.output_file = Path(
            args.output
            or Path(self.bed_file).parent
            / f"{Path(self.bed_file).stem}_terminal_repeats.csv"
        )
        self.threads = args.threads if args.threads else max(1, os.cpu_count() // 2)
        self.name_map = {}
        self.run_temp_dir = Path(
            tempfile.mkdtemp(prefix="ecc_repeats_main_", dir=args.custom_temp_dir)
        )
    ...
    def _parse_input_line(self, line, line_num):
        ...
        original_name, ecc_chr, start_str, end_str = (
            fields[0],
            fields[1],
            fields[2],
            fields[3],
        ...
        try:
            ...
            if ecc_start_0based >= ecc_end_1based_exclusive:
                return None

            # Standardized name for internal tracking / 用于内部追踪的标准化名称
            standardized_name = (
                f"{ecc_chr}_{ecc_start_0based}_{ecc_end_1based_exclusive}"
            )
            self.name_map[standardized_name] = original_name
            return (
                ecc_chr,
                ecc_start_0based,
                ecc_end_1based_exclusive,
                standardized_name,
            )
    ...
    def process(self):
        ...
        blast_params = {
            "min_identity": self.args.min_blast_identity,
            "min_length": self.args.min_blast_length,
            "task": self.args.blast_task,
            "word_size": self.args.blast_word_size,
            "evalue": self.args.blast_evalue,
        }
        analyzer = BlastAnalyzer(
            self.args.genome,
            self.args.fai,
            self.args.extend_outer,
            self.args.extend_inner,
            blast_params,
            self.run_temp_dir,
        )
        ...
        logging.info(
            f"Starting parallel BLAST analysis for {len(all_eccdnas)} eccDNAs using {self.threads} workers."
        )

        with ProcessPoolExecutor(max_workers=self.threads) as executor:
            for i, (direct_df, inverted_df) in enumerate(
                executor.map(analyzer.process, all_eccdnas)
            ):
                if not direct_df.empty:
                    all_direct.append(direct_df)
                if not inverted_df.empty:
                    all_inverted.append(inverted_df)
                if (i + 1) % 100 == 0:
                    logging.info(f"  ... {i + 1}/{len(all_eccdnas)} futures collected.")

        logging.info("BLAST analysis complete. Filtering results...")

        # Process direct repeats / 处理正向重复
        master_direct_df = (
            pd.concat(all_direct, ignore_index=True) if all_direct else pd.DataFrame()
        )
        direct_processor = RepeatResultsProcessor(
            master_direct_df,
            self.run_temp_dir / "direct.csv",
            is_inverted=False,
            tolerance=self.args.breakpoint_tolerance,
        )
        direct_processor.process()

        # Process inverted repeats / 处理反向重复
        master_inverted_df = (
            pd.concat(all_inverted, ignore_index=True)
            if all_inverted
            else pd.DataFrame()
        )
        inverted_processor = RepeatResultsProcessor(
            master_inverted_df,
            self.run_temp_dir / "inverted.csv",
            is_inverted=True,
            tolerance=self.args.breakpoint_tolerance,
        )
        ...
        if not final_dfs:
            ...
            df_combined["eccDNA_orig_name"] = df_combined["eccDNA_name_internal"].map(
                self.name_map
            )
            df_combined.rename(
                columns={"eccDNA_name_internal": "eccDNA_std_name"}, inplace=True
            )
            ...
            logging.info(
                f"✔ Analysis complete. Found {len(df_final)} terminal repeats. Results saved to {self.output_file}"
            )
...
def main():
    ...
    io_group.add_argument(
        ...
        "--input",
        required=True,
        help="Input TSV file. Format: name, chr, start (0-based, inclusive), end (0-based, inclusive).",
    )
    io_group.add_argument(
        "-g", "--genome", required=True, help="Path to the reference genome FASTA file."
    )
    io_group.add_argument(
        "-o",
        "--output",
        default=None,
        help="Output CSV file path. [Default: <input_stem>_terminal_repeats.csv]",
    )
    io_group.add_argument(
        "-t",
        "--threads",
    ...
    region_group.add_argument(
        ...
        type=int,
        default=100,
        help="Base pairs to extend outward from eccDNA ends.",
    )
    region_group.add_argument(
        "--extend_inner",
        type=int,
        default=50,
        help="Base pairs to extend inward from eccDNA ends.",
    )
    ...
    filter_group.add_argument(
        ...
        type=int,
        default=5,
        help="Tolerance (bp) for matching repeat ends to eccDNA breakpoints.",
    )
    filter_group.add_argument(
        "--min_blast_identity",
        type=float,
        default=90.0,
        help="Minimum BLAST percent identity.",
    )
    filter_group.add_argument(
        "--min_blast_length",
        type=int,
    ...
    blast_group.add_argument(
        ...
        help="Task for blastn (e.g., 'blastn', 'blastn-short').",
    )
    blast_group.add_argument(
        "--blast_word_size", type=int, default=7, help="Word size for blastn."
    )
    blast_group.add_argument(
        "--blast_evalue", type=float, default=10.0, help="E-value threshold for blastn."
    )
    ...
    misc_group.add_argument(
        "--fai",
        default=None,
        help="Path to genome .fai index (recommended for accuracy).",
    )
    misc_group.add_argument(
        "--custom_temp_dir",
        default=None,
    ...
    try:
        ...
        logging.error(
            "The pipeline failed due to an unhandled exception.", exc_info=True
        )
        sys.exit(1)
