...
def load_eccdna_data(path: str) -> pd.DataFrame:
    ...
    logger.info(
        f"Loaded {len(df)} eccDNA records from {df['Sample'].nunique()} samples"
    )
    return df
...
def load_degs(path: str) -> pd.DataFrame:
    ...
    logger.info(
        f"DEG range: log2FC from {df['log2fc'].min():.2f} to {df['log2fc'].max():.2f}"
    )
    return df
...
def build_deg_sets(degs: pd.DataFrame, thresholds=(1, 2, 4, 6)):
    ...
    for fc in thresholds:
        ...
        deg_sets[fc] = (up, down)
        logger.debug(
            f"FC={fc}: {len(up)} up-regulated, {len(down)} down-regulated genes"
        )
...
def analyze_overlap(sample_ecc: pd.DataFrame, deg_sets: dict, fc: float):
    ...
    ecc_enriched = set(
        sample_ecc[(sample_ecc.direction == "enrichment") & sample_ecc.significant][
            "gene"
        ]
    )
    ecc_depleted = set(
        sample_ecc[(sample_ecc.direction == "depletion") & sample_ecc.significant][
            "gene"
        ]
    )
...
def plot_gradient(sample: str, grad_df: pd.DataFrame, outdir: Path):
    ...
    fig.suptitle(f"{sample}: eccDNA–DEG association across FC thresholds", fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(
        outdir / f"{sample}_gradient_analysis.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def plot_summary_heatmap(all_grad: pd.DataFrame, outdir: Path):
    ...
    ax = axes[0, 0]
    or_eu_matrix = all_grad.pivot(
        index="sample", columns="fc_threshold", values="OR_enriched_up"
    )
    sns.heatmap(
        or_eu_matrix,
        annot=True,
    ...
    ax = axes[0, 1]
    or_dd_matrix = all_grad.pivot(
        index="sample", columns="fc_threshold", values="OR_depleted_down"
    )
    sns.heatmap(
        or_dd_matrix,
        annot=True,
    ...
    ax = axes[1, 0]
    p_eu_matrix = all_grad.pivot(
        index="sample", columns="fc_threshold", values="p_enriched_up"
    )
    ...
    ax = axes[1, 1]
    p_dd_matrix = all_grad.pivot(
        index="sample", columns="fc_threshold", values="p_depleted_down"
    )
...
def plot_overlap_summary(all_grad: pd.DataFrame, outdir: Path):
    ...
    ax2.bar(
        fc2_data["sample"], fc2_data["n_depleted_down"], color="darkblue", alpha=0.7
    )
...
def run(args):
    ...
    for s in samples:
        ...
        summary_df = grad_df.drop(
            columns=[col for col in grad_df.columns if col.startswith("genes_")]
        )
    ...
    for _, row in sig_associations.iterrows():
        if row["p_enriched_up"] < 0.05:
            logger.info(
                f"  {row['sample']} @ FC={row['fc_threshold']}: "
...
def cli():
    ...
    p.add_argument("--outdir", default="eccdna_deg_results", help="Output directory")
    p.add_argument(
        "--fc",
        default="1,2,4,6",
