...
class DataProcessor:
    ...
    def process_all_files(self, input_dir):
        ...
        for filepath in gff_files:
            ...
            logger.info(
                f"处理文件: {filepath.name} (样本: {sample_name}, 类型: {sample_type})"
            )
            records = self.parser.parse_file(filepath)

            # 存储数据
            if sample_name not in data:
                data[sample_name] = {"group": sample_group, "Mecc": [], "Uecc": []}
    ...
    def calculate_basic_stats(self, data):
        ...
        for sample_name, sample_data in data.items():
            for ecc_type in ["Mecc", "Uecc"]:
                ...
                stat["top_motif_counts"] = ",".join(
                    top_motifs.values.astype(str).tolist()
                )
                ...
                summary.append(
                    {
                        ...
                        "avg_coverage_per_eccdna": coverage_stats[
                            "match_length"
                        ].mean(),
                    }
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Phase 1: eccDNA转座子数据解析和基础统计"
    )
    ...
    with open(output_dir / "data_summary.txt", "w") as f:
        ...
        f.write(
            f"Mecc记录数: {basic_stats[basic_stats['type']=='Mecc']['total_records'].sum():,}\n"
        )
        f.write(
            f"Uecc记录数: {basic_stats[basic_stats['type']=='Uecc']['total_records'].sum():,}\n\n"
        )
        ...
        for group in ["AT", "U87", "HeLa", "ZM"]:
            ...
            if not group_data.empty:
                ...
                f.write(
                    f"  Mecc: {group_data[group_data['type']=='Mecc']['total_records'].sum():,}\n"
                )
                f.write(
                    f"  Uecc: {group_data[group_data['type']=='Uecc']['total_records'].sum():,}\n"
                )
    ...
    print(f"\n各组记录数:")
    group_stats = (
        basic_stats.groupby(["group", "type"])["total_records"]
        .sum()
