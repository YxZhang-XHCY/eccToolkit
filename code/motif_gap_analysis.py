...
for sample, group in df.groupby(sample_col):
    ...
    gap_table.append(
        [
            ...
            gap_size,
            motif_counts.index[max_gap_idx],
            motif_counts.iloc[max_gap_idx],
            motif_counts.index[max_gap_idx + 1],
    ...
    # 标记点文本
    ax.annotate(
        f"{motif_counts.index[max_gap_idx]}\n{motif_counts.iloc[max_gap_idx]}",
    ...
    ax.annotate(
        f"{motif_counts.index[max_gap_idx+1]}\n{motif_counts.iloc[max_gap_idx+1]}",
...
print(
    "分析完成！每个样本的rank plot图已保存为PNG，断层信息已保存为 motif_gap_summary.csv"
)
