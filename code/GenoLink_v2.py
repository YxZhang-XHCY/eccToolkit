...
def filter_chr_specific_groups(
    df: pd.DataFrame, target_chr: str = "chr1"
) -> pd.DataFrame:
...
def validate_input_data(df: pd.DataFrame) -> bool:
    ...
    if not pd.api.types.is_numeric_dtype(
        df["eStart"]
    ) or not pd.api.types.is_numeric_dtype(df["eEnd"]):
...
def process_group_with_color_scheme(
...
def process_file_with_chr_filter(
    input_file: str,
    output_file: str,
    target_chr: str = "chr1",
    color_scheme: str = "highlight_largest",
    ...
    if target_chr:
        ...
        if df_filtered.empty:
            ...
            # 创建空的输出文件
            pd.DataFrame(
                columns=["Chr1", "Start1", "End1", "Chr2", "Start2", "End2", "RGB"]
    ...
    # 准备参数
    process_args = [
        (name, group_df, largest_group_name, color_scheme)
...
def main():
    parser = argparse.ArgumentParser(
        description="生成基因组链接关系图数据（支持染色体筛选和颜色优化）"
    )
    ...
    parser.add_argument(
        "--color-scheme",
        choices=["highlight_largest", "random"],
        default="highlight_largest",
        help="颜色方案：highlight_largest(突出最大组), random(随机颜色)",
    )
    parser.add_argument(
        "--seed", type=int, default=None, help="随机种子（用于可重复的颜色生成）"
    )
