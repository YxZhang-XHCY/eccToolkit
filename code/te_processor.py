...
def process_te_data(te_file, ref_file, output_file):
    ...
    te_df['motif_percent'] = np.where(
        (pd.notnull(te_df['motif_length'])) & (pd.notnull(te_df['seq_length'])) & (te_df['seq_length'] > 0),
    ...
    te_df['anno_Percent'] = np.where(
        (pd.notnull(te_df['start'])) & (pd.notnull(te_df['end'])) & 
        (pd.notnull(te_df['seq_length'])) & (te_df['seq_length'] > 0),
