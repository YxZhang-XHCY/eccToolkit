...
def perform_gradient_fc_analysis_single_sample(
    eccdna_df, degs_df, sample_name, fc_thresholds=[1, 2, 4, 6]
):
    """对单个样本执行梯度FC阈值分析"""
    results_summary = []
    ...
    eccdna_enriched_all = set(
        sample_eccdna[
            (sample_eccdna["direction"] == "enrichment")
            & (sample_eccdna["significant"] == True)
        ]["gene"]
    )
    eccdna_depleted_all = set(
        sample_eccdna[
            (sample_eccdna["direction"] == "depletion")
            & (sample_eccdna["significant"] == True)
        ]["gene"]
    )
    ...
    for fc in fc_thresholds:
        ...
        results, stats, _, _, _, _ = analyze_eccdna_degs_overlap(
            sample_eccdna, degs_df, fc_threshold=fc
        )
...
def plot_sample_specific_analysis(sample_name, gradient_df, eccdna_df, degs_df):
    ...
    fig.suptitle(
        f"Sample {sample_name}: eccDNA-DEG Association Across FC Thresholds",
        fontsize=16,
    )
    ...
    if "enriched_up_OR" in gradient_df.columns:
        ...
        ax.plot(
            gradient_df.loc[mask, "fc_threshold"],
            gradient_df.loc[mask, "enriched_up_OR"],
            "o-",
            label="Enriched-Up",
            color="red",
        )
    if "depleted_down_OR" in gradient_df.columns:
        ...
        ax.plot(
            gradient_df.loc[mask, "fc_threshold"],
            gradient_df.loc[mask, "depleted_down_OR"],
            "s-",
            label="Depleted-Down",
            color="blue",
        )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"{sample_name}_gradient_analysis.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    results, stats, eccdna_enriched, eccdna_depleted, degs_up, degs_down = (
        analyze_eccdna_degs_overlap(sample_eccdna, degs_df, fc_threshold=2)
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"{sample_name}_venn_diagrams.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_sample_comparison_heatmap(all_sample_results):
    ...
    # 基因数量热图
    sns.heatmap(
        comparison_df[["Enriched∩Up", "Enriched∩Down", "Depleted∩Up", "Depleted∩Down"]],
        annot=True,
        fmt="d",
        cmap="YlOrRd",
        ax=ax1,
    ...
    # Odds Ratio热图
    sns.heatmap(
        comparison_df[["Enriched-Up OR", "Depleted-Down OR"]],
        annot=True,
        fmt=".2f",
        cmap="RdBu_r",
        center=1,
...
def analyze_cross_sample_consistency(eccdna_df, degs_df, samples):
    ...
    for sample in samples:
        ...
        results, _, _, _, _, _ = analyze_eccdna_degs_overlap(
            sample_data, degs_df, fc_threshold=2
        )
    ...
    for category in sample_genes.keys():
        ...
        for gene in all_genes:
            count = sum(
                1
                for sample_set in sample_genes[category].values()
                if gene in sample_set
            )
            gene_frequency[gene] = count

        consistency_stats[category] = {
            "total_unique_genes": len(all_genes),
            "genes_in_all_samples": sum(
                1 for count in gene_frequency.values() if count == len(samples)
            ),
            "genes_in_majority": sum(
                1 for count in gene_frequency.values() if count > len(samples) / 2
            ),
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="eccDNA富集/耗竭与RNA-seq差异表达基因关联分析 - 简化版本",
...
使用示例:
    ...
    python eccdna_deg_correlation_analysis_simple.py -e enrichment.csv -d degs.txt -o output/ --fc-threshold 2
        """,
    )

    parser.add_argument(
        "-e", "--eccdna", required=True, help="eccDNA富集分析结果文件路径 (CSV format)"
    )
    parser.add_argument(
        "-d", "--degs", required=True, help="差异表达基因文件路径 (TSV format)"
    )
    parser.add_argument("-o", "--output-dir", required=True, help="输出目录路径")
    parser.add_argument(
        "--fc-threshold", type=float, default=1.0, help="Log2FC阈值 (default: 1.0)"
    )
...
def main():
    ...
    # 检查输入文件是否存在
    if not os.path.exists(args.eccdna):
        print(f"错误: eccDNA文件不存在: {args.eccdna}")
        return

    if not os.path.exists(args.degs):
        print(f"错误: DEGs文件不存在: {args.degs}")
        return
    ...
    # 加载数据
    print("Loading data...")
    eccdna_df = load_eccdna_data(args.eccdna)
    degs_df = load_degs_data(args.degs)
    ...
    for sample in samples:
        ...
        # 获取FC=2的详细统计
        sample_data = eccdna_df[eccdna_df["Sample"] == sample]
        _, stats_fc2, _, _, _, _ = analyze_eccdna_degs_overlap(
            sample_data, degs_df, fc_threshold=2
        )
        all_sample_stats[sample] = stats_fc2

        # 创建样本特定的图表和报告
        output_dir = plot_sample_specific_analysis(
            sample, gradient_results, eccdna_df, degs_df
        )
        generate_sample_report(sample, gradient_results, stats_fc2, output_dir)

        # 保存样本特定的梯度分析结果
        gradient_results.to_csv(
            os.path.join(output_dir, f"{sample}_gradient_results.csv"), index=False
        )
    ...
    with open("comprehensive_multi_sample_report.txt", "w") as f:
        ...
        for category, stats in consistency_stats.items():
            f.write(f"\n{category}:\n")
            f.write(
                f"  Total unique genes across all samples: {stats['total_unique_genes']}\n"
            )
            f.write(f"  Genes found in all samples: {stats['genes_in_all_samples']}\n")
            f.write(
                f"  Genes found in majority of samples: {stats['genes_in_majority']}\n"
            )
            ...
            if stats["genes_in_all_samples"] > 0:
                consistent_genes = [
                    g
                    for g, count in stats["gene_frequency"].items()
                    if count == len(samples)
                ]
                f.write(
                    f"  Consistent genes: {', '.join(sorted(consistent_genes)[:10])}"
                )
        ...
        f.write("-" * 50 + "\n")
        for sample in samples:
            fc2_data = all_sample_results[sample][
                all_sample_results[sample]["fc_threshold"] == 2
            ].iloc[0]
            f.write(f"\n{sample}:\n")
            ...
            if "enriched_up_OR" in fc2_data and not pd.isna(fc2_data["enriched_up_OR"]):
                f.write(
                    f", OR={fc2_data['enriched_up_OR']:.2f}, p={fc2_data['enriched_up_pvalue']:.3e}"
                )
            f.write("\n")
            f.write(f"  Depleted-Down association: n={fc2_data['n_depleted_down']}")
            if "depleted_down_OR" in fc2_data and not pd.isna(
                fc2_data["depleted_down_OR"]
            ):
                f.write(
                    f", OR={fc2_data['depleted_down_OR']:.2f}, p={fc2_data['depleted_down_pvalue']:.3e}"
                )
            f.write("\n")
    ...
    if len(samples) >= 2:
        ...
        for idx, (cat, title) in enumerate(zip(categories, titles)):
            ...
            results1, _, _, _, _, _ = analyze_eccdna_degs_overlap(
                sample1_data, degs_df, fc_threshold=2
            )
            results2, _, _, _, _, _ = analyze_eccdna_degs_overlap(
                sample2_data, degs_df, fc_threshold=2
            )

            venn2([results1[cat], results2[cat]], set_labels=[sample1, sample2], ax=ax)
