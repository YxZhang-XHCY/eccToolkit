...
class CNVEccDNAAnalysis:
    ...
    def analyze_cnv_enrichment(self, eccdna_df, cnv_df, n_shuffles=1000):
        ...
        if results:
            ...
            results_df.to_csv(
                self.results_dir / f"{self.prefix}_cnv_enrichment_results.csv",
                index=False,
            )
    ...
    def analyze_cnv_detailed(self, eccdna_df, cnv_df):
        ...
        cnv_detailed_df.to_csv(
            self.results_dir / f"{self.prefix}_cnv_detailed_stats.csv", index=False
        )
        ...
        stats_results.to_csv(
            self.results_dir / f"{self.prefix}_cnv_statistical_tests.csv", index=False
        )
    ...
    def export_null_distributions(self, null_distributions):
        ...
        df.to_csv(
            self.plot_data_dir / f"{self.prefix}_cnv_null_distributions.csv",
            index=False,
        )
    ...
    def export_cnv_plot_data(self, cnv_df):
        ...
        # Export raw data
        cnv_df.to_csv(
            self.plot_data_dir / f"{self.prefix}_cnv_eccdna_density_data.csv",
            index=False,
        )
        ...
        summary_stats.to_csv(
            self.plot_data_dir / f"{self.prefix}_cnv_summary_statistics.csv",
            index=False,
        )
    ...
    def plot_null_distribution(self, null_counts, observed, title):
        ...
        plt.savefig(
            self.figures_dir / f"{self.prefix}_null_dist_{safe_title}.png", dpi=300
        )
        plt.close()
    ...
    def plot_quality_control(self, eccdna_df):
        ...
        chr_dist_data.to_csv(
            self.plot_data_dir / f"{self.prefix}_chromosome_distribution.csv",
            index=False,
        )
        ...
        if "eLength" in eccdna_df.columns:
            ...
            size_data.to_csv(
                self.plot_data_dir / f"{self.prefix}_size_distribution.csv", index=False
            )
...
"""
    ...
    def run_analysis(self):
        ...
        try:
            ...
            cnv_results = self.analyze_cnv_enrichment(
                eccdna_df, cnv_df, self.n_shuffles
            )
...
Examples:
  ...
  %(prog)s -i eccDNA.csv -c CNV.bed -p sample1 -w ./results/
    ...
    # Optional arguments
    parser.add_argument(
        "-p", "--prefix", default="eccDNA", help="Output file prefix (default: eccDNA)"
    )
    parser.add_argument(
        "-g",
        "--genome",
