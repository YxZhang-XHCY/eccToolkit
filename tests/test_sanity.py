...
from ecctoolkit.simulate._wip_full.rca_readsim.library.hifi import HiFiLibraryGenerator
from ecctoolkit.simulate._wip_full.rca_readsim.debranch import LinearMoleculePool, Debrancher
...
def test_linear_molecule_get_subsequence_matches_slice():
    ecc_a = _make_eccdna("ecc_a", 37)
    ecc_b = _make_eccdna("ecc_b", 23)
    register_ecc_length(ecc_a.id, ecc_a.length)
    register_ecc_length(ecc_b.id, ecc_b.length)
    ecc_db = {ecc_a.id: ecc_a, ecc_b.id: ecc_b}

    mol = LinearMolecule(
        molecule_id="mol_multi",
        segments=[
            Segment(
                ecc_id=ecc_a.id,
                ecc_offset=5,
    ...
    full = mol.get_sequence(ecc_db)

    for start, length in [(0, 10), (5, 12), (20, 20), (40, 100), (-5, 8), (999, 3)]:
        if start < 0:
            expected = full[0:length]
        else:
            expected = full[start:start + length]
        assert mol.get_subsequence(ecc_db, start, length) == expected
